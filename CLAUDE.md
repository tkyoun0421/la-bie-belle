...
PRD와 ADR을 먼저 세우고 `docs/plan.md`의 task로 쪼갠다. 기능 개발과 디자인은 병렬로 가고, 디자인이 완료된 화면부터 퍼블리싱이 붙는다.
...
회차가 끝나면 총괄이 `docs/log/`에 로그를 쓰고 `docs/plan.md`의 task 상태를 갱신한다. 로그를 쓰는 주체는 총괄뿐이다.
