...
- `docs/backlog.md` — task 보드. 완료 조건은 행이 링크하는 spec(기능)이나 plan(비기능)에 산다
...
- `docs/proposals/` — 단계에 걸치는 변경 방향의 검토 제안. 작성 기준·상태·목록은 그 안 `README.md`, 근거는 ADR-008
...
- 구조 설계 파이프라인: 인터뷰(`grill-me`, 한 라운드 한 질문) → 초안 → `architecture-advisor` 검토 → 총괄이 `architecture/`에 씀. 조언자는 `architecture/README.md`의 폴더 하나씩 보고 결정하지 않는다
