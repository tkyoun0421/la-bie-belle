...
- 구조 설계 파이프라인: 인터뷰(`grill-me`, 한 라운드 한 질문) → 초안 → `architecture-advisor` 검토 → 총괄이 `architecture/`에 씀. 조언자는 `architecture/` 아래 폴더 하나씩 보고 결정하지 않는다 — 폴더 목록은 그 안 `README.md`가 정본이다
