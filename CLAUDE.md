...
이 저장소는 사람(태관)과 총괄 세션이 끌고 간다. 상주 역할 세션은 없다.

작업 트리를 갈래로 나눈다. 저장소 본체에서는 디자인과 문서만 하고 코드는 별도 작업 트리에서 간다. 화면을 정하는 일과 그것을 세우는 일이 서로를 기다리지 않게 하려는 것이다. 트리마다 세션이 하나씩 붙고 둘 다 총괄이다.

그래서 본체 트리는 `src/`와 `tests/`에 손대지 않는다. 코드 쪽에서 고칠 것을 발견하면 직접 고치지 말고 문서에 적어 개발 트리로 넘긴다. 반대로 개발 트리는 디자인 결정을 내리지 않고 `docs/design-system/`을 따른다.
