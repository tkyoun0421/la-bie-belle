...
description: 이 프로젝트의 AI 하네스 스킬 구조를 파악하거나 어떤 세부 스킬을 써야 하는지 고를 때 사용한다. 제품 전체 PRD가 필요하면 product-prd를 사용하고, 작업 중 떠오른 수시 작업을 즉시 이슈화하려면 ai-harness-capture를 사용한다. PRD 기반으로 다음 할 일을 정할 때는 ai-harness-idea를 사용하고, 선택된 큰 아이디어를 이슈 초안으로 나눌 때는 ai-harness-plan을 사용한다. 생성된 이슈를 상세 구현 스펙으로 구체화할 때는 ai-harness-spec을 사용한다. 실제 구현, 검증, 리뷰, 대시보드, 하네스 평가, 초안 PR 작업은 각 세부 스킬을 직접 사용한다.
---
...
- PRD 기반 작업은 `ai-harness-idea`로 다음 할 일을 정하고, `ai-harness-plan`으로 이슈 초안을 만든 뒤 사용자 승인을 받아야 한다.
- 작업 중 떠오른 수시 발생 작업은 `ai-harness-capture`로 즉시 이슈 초안을 만들고, 단일 GitHub Project의 Inbox에 넣는다.
- 승인 없이 이슈를 생성하지 않는다.
- 제품 로드맵은 하나의 GitHub Project에서 관리하고, MVP 구분은 Project의 `MVP` 필드로 한다.
