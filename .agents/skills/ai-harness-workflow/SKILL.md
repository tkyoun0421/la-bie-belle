...
description: 이 프로젝트의 AI 하네스 스킬 구조를 파악하거나 어떤 세부 스킬을 써야 하는지 고를 때 사용한다. 제품 전체 PRD가 필요하면 product-prd를 사용하고, 작업 중 떠오른 수시 작업은 .agents/inbox.md에 자동 캡처하며 이슈 승격이 필요할 때 ai-harness-capture를 사용한다. PRD 기반으로 다음 할 일을 정할 때는 ai-harness-idea를 사용하고, 선택된 큰 아이디어를 이슈 초안으로 나눌 때는 ai-harness-plan을 사용한다. 생성된 이슈를 상세 구현 스펙으로 구체화할 때는 ai-harness-spec을 사용한다. 실제 구현, 검증, 리뷰, 대시보드, 하네스 평가, PR 생성 작업은 각 세부 스킬을 직접 사용한다.
---
