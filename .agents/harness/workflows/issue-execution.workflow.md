...
- 하네스 산출물의 설명 문장, 근거, 감점 사유, 요약, 권장 다음 작업은 기본 한국어로 작성한다.
- 파일명, JSON 키, 상태값(`PASS`, `REWORK`, `FAIL`), 명령어, 코드 식별자, 라이브러리명은 원문 영어를 유지할 수 있다.
- 사용자가 영어 작성을 명시적으로 요청한 경우에만 영어 설명 문장을 사용한다.
- 사용자 결정이 필요한 항목은 추정으로 확정하지 않는다.

## 단계

Pre-Issue. 이슈 생성 승인
   - PRD 기반 새 GitHub Issue가 필요한 경우 먼저 계획과 이슈 제목, 본문, 라벨을 사람에게 제시한다.
   - 작업 중 떠오른 수시 발생 작업은 `.agents/inbox.md`에 자동 캡처하고, 명확한 결정 항목만 `ai-harness-capture`로 이슈 초안을 만든다.
   - inbox 항목의 분류, PR 내 처리, 제거 기준은 `.agents/harness/inbox-policy.md`를 따른다.
   - 사람의 승인 전에는 GitHub Issue를 생성하지 않는다.
...
2. Spec
   ...
   - 원본 Issue, 코멘트, `task-spec.md`, `plan.md`에 `확인 필요`, `결정 필요`, `구현 전에`, `스펙 확정`, `열린 질문` 또는 의미상 같은 미확정 결정이 있으면 `spec.md`를 만들지 않는다.
   - 미확정 결정이 있으면 먼저 사용자에게 한 가지 질문을 하고, 답변으로 확정된 내용만 `spec.md`에 기록한다.
...
5. Refactor
   - Green 이후 테스트가 통과하는 상태에서 조건부로 수행한다.
   - 동작 변경 없이 구조만 개선하거나, 리팩터링 생략 사유를 기록한다.
...
7. 리뷰
   ...
   - `review-score.json`의 근거, 감점 사유, 강점, 약점, 권장 다음 작업은 한국어로 작성한다.
...
8. Rework Loop
   ...
   - 재작업 후 검증과 리뷰를 다시 실행한다.
   - 기본 재작업 횟수는 1회다.

9. PR
   - 리뷰 결정이 `PASS`일 때만 GitHub Draft 상태가 아닌 일반 PR을 생성한다.
   - PR 생성 전 원본 이슈 완료 기준, 작업 중 추가 요청사항, 현재 이슈 관련 inbox 항목을 확인한다.
   - 현재 이슈 관련 inbox 항목은 `included-in-pr`, `promoted-to-issue`, `deferred` 중 하나로 처리하고, 남기는 기록은 PR 본문, GitHub Issue, run 산출물에 남긴 뒤 inbox에서 제거한다.
...
10. 하네스 평가자
   ...
   - `harness-health-score.json`, `harness-improvements.md`, 개선 제안 설명은 한국어로 작성한다.
   - 낮은 점수 항목은 개선안으로 기록한다.

## 중단 조건

- 계획 단계에서 구현 방향을 결정할 수 없는 핵심 질문이 남은 경우.
- Spec 단계에서 미확정 결정이 남아 있는데 사용자 답변이 없는 경우.
