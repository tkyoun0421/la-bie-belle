export function blockedCommitPolicy(taskId, stagedPaths) {
  const allowed = new Set([
    ...
    `.agents/runs/${taskId}/decision-signal.json`
  ]);
  const required = [...allowed].filter((path) => !path.endsWith("/decision-signal.json"));
  const missing = required.filter((path) => !stagedPaths.includes(path));
