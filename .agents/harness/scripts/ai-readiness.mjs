...
const facts = {
  ...
  "worktree runner": exists(".agents/harness/scripts/run.mjs"), "attempt limit": Number(JSON.parse(readFileSync(join(root, ".agents/harness/config.json"), "utf8")).max_attempts) === 3, "run evidence": exists(".agents/runs"),
  ...
  "commit message guard": exists(".githooks/commit-msg"), "no deploy runner": !/vercel|git\s+push/i.test(readFileSync(join(root, ".agents/harness/scripts/run.mjs"), "utf8")), "spec references": tasks.length > 0 && tasks.every((task) => Array.isArray(task.spec_refs) && task.spec_refs.length > 0)
};
