...
- **푸시 권한 요청 진입점은 두 곳이다.** 첫 근무 신청 저장 성공 후 사전 안내(기존 정본 유지)와 더보기 알림 설정 — 인터뷰 중 단일 트리거 답과 기존 정본의 충돌을 드러내 병행으로 확정했다. 기기마다 따로 켠다. — [PRD 알림 절](PRD.md) · [WORKER-FLOWS 알림과 푸시 권한 절](design/WORKER-FLOWS.md) (2026-08-16 기획)
- **테스트 푸시 발송은 dev 스크립트로 한다.** 관리자 테스트 버튼 같은 제품 표면은 추가하지 않는다 — P4-T08 소비자 전의 체크리스트 전용 수단. — [phase P4-T02 절](../execution/phases/04-changes-and-notifications.md) (2026-08-16 기획)
- **실기기 수신 확인은 P7-T06 리허설로 이월한다.** P4-T02는 실기기 체크리스트 문서 작성까지가 완료 조건이고, Android·홈 화면 설치 iPhone 확인은 전체 여정 리허설에서 그 문서로 수행한다. — [phase P4-T02 절](../execution/phases/04-changes-and-notifications.md) · [phase P7-T06 절](../execution/phases/07-pilot-and-launch.md) (2026-08-16 기획)

### 모집·변경 알림 (P4-T03)

- **모집 오픈 알림은 일괄 오픈 1회당 1건이다.** 관리자가 여러 근무일을 한 번에 열어도 근무자는 알림 한 건을 받고, 클릭하면 일정 화면으로 간다 — 날짜별 도배를 만들지 않는다. — [PRD 알림 절](PRD.md) (2026-08-16 기획)
- **모집 오픈 수신자는 활성 근무자 전원이다.** 모집을 볼 수 있는 기준(profiles.status active인 worker)과 동일 — 휴면·탈퇴·승인 대기는 자동 제외되고 관리자는 받지 않는다. — [PRD 알림 절](PRD.md) (2026-08-16 기획)
- **변경 알림은 시간·예식·배정 3종에서만 발생한다.** 필요 인원 변경은 관리 내부 사안이라 알림을 만들지 않는다. — [PRD 알림 절](PRD.md) (2026-08-16 기획)
- **변경 알림 수신자는 변경 전후 배정자·교육생의 합집합이다.** 배정에서 빠진 사람도 받아 모르고 출근하는 사고를 막는다. 범위는 스케줄 전체 — 배정 변경 시 다른 포지션의 동료도 받는다(2026-08-17 검증 단계에서 포지션 한정 해석을 기각하고 확정). 확정 스케줄 취소 알림 여부는 P4-T04 기획 소유로 이월. — [PRD 알림 절](PRD.md) · [phase P4-T04 절](../execution/phases/04-changes-and-notifications.md) (2026-08-16 기획)
