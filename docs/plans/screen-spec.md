...
### 1. 행사 생성
1. 팀장이 행사 생성 화면을 연다.
2. 달력에서 하나 이상의 날짜를 선택한다.
3. 필요하면 템플릿 기본값을 불러와 시간과 슬롯을 채운다.
4. 선택한 날짜에 생성될 스케줄을 확인한다.
5. 모집을 연다.

### 2. 신청과 배정
1. 팀원이 달력에서 열린 날짜를 여러 개 고른다.
2. 선택한 날짜에 포함된 열린 스케줄 목록을 보고 신청하거나 취소한다.
3. 팀장이 신청 목록을 확인한다.
4. 중복 배정 경고가 있으면 확인 후 배정한다.
5. 확정 알림을 보낸다.
