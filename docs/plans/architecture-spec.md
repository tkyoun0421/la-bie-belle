...
- 템플릿의 슬롯 기본값은 `event_template_position_slots`에 저장하고 행사 생성 시 `event_position_slots`로 복사한다.
...
- route는 server component로 시작한다.
- admin 화면은 server page에서 데이터를 읽고 `HydrationBoundary`로 seed를 내려준다.
- `flows/*`는 server shell을 유지하고, 입력 폼/검색/즉시 반응이 필요한 부분만 client island로 분리한다.
- screen 전체를 client component로 올리는 방식은 기본값으로 쓰지 않는다.
