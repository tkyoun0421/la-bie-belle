...
## 2026-08-14 정정 — 교차 검증 F-03: AC7 전이는 e2e로 확인한 적이 없었다

위 「위험 기반 테스트 매트릭스 반영」의 "7 담당자 없음" 절이 "정식을 모두 뺀 뒤에도 교육생이 남아
화면에 뜨는 실제 흐름을 e2e로 확인했다"고 적었으나, 정확히는 **e2e가 확인한 것은 제거 호출 이후의
최종 화면 상태(교육생 표시·`담당자 없음` 배지)뿐**이었다 — 시딩 단계에서 처음부터 정식 배정자가
없는 포지션에 교육생만 심어 둔 뒤 화면을 렌더링했을 뿐, "정식 배정자가 있던 포지션에서 전원 제거하는
호출 자체가 교육생 행을 건드리지 않는다"는 전이(transition)는 어느 계층에서도 실제로 실행해 확인한
적이 없었다. 이 표현은 부정확했다.

교차 검증(critical F-01과 함께 묶인 high F-03)이 이 구멍을 지적했고, revision 3 수정 라운드에서
`supabase/tests/20-assignment-trainees.test.sql`에 pgTAP 전이 단언을 추가했다 — 포지션에 정식
배정자(F12)와 교육생(F13)을 함께 둔 뒤, 정식 배정자만 전원 제거하는 3-인자 호출(네번째 인자 없음)을
실행하고, 그 직후 F12의 정식 배정이 실제로 사라졌는지와 F13의 교육생 행이 그대로 남아 있는지를 각각
단언했다. 근거는 `docs/execution/runs/P3-T05/tdd.json`의 pgTAP RED→GREEN 쌍(2026-08-14).
