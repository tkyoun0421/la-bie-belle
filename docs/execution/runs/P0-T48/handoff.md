...
## 2026-08-19 · 4단계 교차 검증 결과 — 확정 16, 기각 2, 점수 70

`opus` 서브 에이전트와 `codex exec -s read-only` 두 리뷰어를 독립으로 돌리고 조정자가 병합했다. 기준 커밋 `8a0c84ed89ad8f36037a0150e2c7ecfe31ae5a5f`, 대상 172 파일 `+19,679 −3,017`. 결과 정본은 [`docs/execution/reviews/P0-T48-review.json`](../../reviews/P0-T48-review.json)이다.

**둘 다 인정한 것만 확정했다.** 한쪽만 든 발견은 상대에게 되물어 근거를 받았고, 근거가 서면 확정, 반박이 서면 기각이다. 중요도가 갈리면 계약대로 높은 쪽을 택했다.

| 확정 | 중요도 | 무엇 |
| --- | --- | --- |
| F-01 | high | 홈의 `<Suspense>` 다섯이 클라이언트 경계 안이라 스트리밍이 안 일어난다 |
| F-02 | high | 오늘 근무 CTA에 목적지가 없고 오프라인 상태를 안 본다 |
| F-03 | high | `PayBlock.tsx:42`가 핸들러 없는 `role="button"`을 걸고 진짜 버튼을 중첩한다 |
| F-04 | high | 남은 시간 카운트다운이 첫 렌더에 멈춰 있다 |
| F-05 ~ F-14 | medium | index 범위 불일치 · `ui` 판정 잔존 · 마스킹 자릿수 · reduced-motion 크로스페이드 · 700 단언 범위 · persist rehydrate · 실패 한 줄 두 벌 · animationend 이름 가드 · 알림 슬롯 첫 페인트 · 알림 진입 잘림 |
| F-15 · F-16 | low | 날짜별 localStorage 키 누적 · 호출부 없는 API 표면 둘 |

점수 **70** — `code_quality` 62 · `tests` 63 · `security` 85 · `performance` 74 · `architecture` 66.

### 기각 둘 — 근거를 받고 물렀다

**codex B1 「`error.tsx`가 `reset`을 받아야 한다」.** opus가 `node_modules/next/dist/client/components/error-boundary.d.ts:3-7`을 들어 반박했다 — Next 16은 `reset`과 `retry`를 **둘 다** 넘기고, 공식 문서 `error.md:25-31`이 `{ error, retry }`를 표준 형태로 쓴다. 지금 코드가 맞다.

**opus A8 「`animationend` `useEffect`가 두 `ui` 파일에 중복이라 공용 훅으로 빼야 한다」.** codex가 `DEVELOPMENT.md:124`를 들어 반박했다 — `ui`가 이벤트 배선을 소유하는 것이 규약이고, 인수 조건 21이 금지한 것은 `useState`·`useReducer`이지 `useEffect`가 아니다. 두 자리의 애니메이션 이름과 정리 시점이 달라 공용화가 오히려 결합을 만든다.

### opus가 스스로 뒤집은 것 — F-07

1라운드에서 opus는 마스킹 자릿수 보존을 「봉인된 결정」으로 보고 발견에서 뺐다. 되묻자 `FOUNDATIONS.md:140`(「고정 폭 더미 위에 걸어 자릿수가 드러나지 않게 한다」)이 `NOTES.md:1233`(「숫자만 0으로 바꾼 같은 길이 문자열」)과 **정면으로 부딪힌다**는 것을 찾았고, RADIO 불변 규칙상 L3가 이기므로 판정을 뒤집었다. 실제 값은 DOM에 안 들어가므로 유출은 아니고, 드러나는 것은 자릿수다. **테스트가 현재 동작을 성공 조건으로 고정**하고 있어 고칠 때 테스트도 함께 뒤집어야 한다.

### F-05는 즉시 고쳤다

`index.jsonl`의 P0-T48 `verification` 셋째 항목이 revision 14로 빠진 일정 화면을 아직 요구하고 있었다. 홈만 남기고, `check_ids`의 `home-schedule-republished`를 `home-republished`로 바꿨다. **새 결정이 아니라 index가 승인된 범위를 뒤늦게 따라간 것이다.** 봉인된 `P0-T48-radio.md:31`의 「예정 check IDs」 줄은 그대로 둔다 — revision 14가 더한 이관 목록 표가 이미 그 위에서 차이를 설명한다.

### high 넷의 처리 — 셋은 이번에 고치고 둘은 못 고친다

수정 라운드의 기본 범위는 `critical`·`high`다(`REVIEW.md:104`). 넷을 갈라 보니 셋은 **확정 시안이 이미 정해 둔 동작을 구현이 빠뜨린 것**이라 새 결정 없이 봉인 경로 안에서 고쳐진다.

- **F-04** — `confirmed/home.html:2684`가 `setInterval`로 1초마다 시계를 다시 그린다. 살아 움직이는 시계가 확정 동작인데 `HomeView.tsx:56`이 프롭 `now`로 한 번만 계산한다.
- **F-02의 오프라인 절반** — `confirmed/home.html:2646-2651`이 오프라인이면 비활성이고 라벨이 「연결되면 인증할 수 있어요」로 바뀐다고 정한다. 구현은 `before-window`만 본다.
- **F-03** — `confirmed/home.html:1985`가 정한 상호작용은 금액 여닫기 하나뿐이고 그건 안쪽 진짜 버튼이 한다. 바깥 행은 대화형이 아니다.

**못 고치는 둘은 시안과 RADIO가 스스로 미룬 것이다.**

- **F-02의 이동 절반** — `confirmed/home.html:1989` 「출근 인증하기는 위치 확인 화면으로 넘어간다 … 그 화면은 **P0-T49**가 그린다」. 목적지가 아직 없다.
- **F-01** — 홈은 `home.mock.ts`를 먹는 동기 목 데이터다. RADIO 「화면」 절이 「새 쿼리를 만들지 않고 기존 쿼리를 고치지도 않는다」로 못 박아, **`<Suspense>`를 어디로 옮겨도 흘려보낼 것이 없다.** 진짜 데이터가 붙는 task가 함께 풀 문제다. `home.mock.ts`를 `ui`에서 `model`로 옮기는 후속 task가 이미 이 자리를 지난다.

### verify를 다섯 번 돌려 얻은 것 — 로컬 잔여 데이터가 e2e를 무너뜨리고 있었다

수정 라운드 GREEN 뒤 `pnpm verify`가 연달아 실패했다. 원인이 하나가 아니었다.

**첫째, 머신 포화.** load average가 20~36을 오갔고(8코어) 순수 모킹 테스트인
`list-recruitment-schedules.test.ts`가 5000ms 기본 타임아웃을 넘겨 두 번 죽었다. 단독 실행은 756ms다.
부하의 출처는 이 세션 밖이다 — VM 하나가 나흘째 83% CPU를 물고 있고 Orca·Docker·다른 claude 세션·
playwright-mcp 여섯이 함께 떠 있다. **1962개 중 하나만 5초를 넘겨도 verify 전체가 빨간불이 된다.**
재시도 외에 이 저장소가 가진 수단이 없다.

**둘째, 중단된 실행이 남긴 DB 잔여.** `schedules`에 948행이 쌓여 있었고 그중 888행이 5년 뒤 이후
날짜였다. 시드는 `schedules`를 만들지 않으니 전부 e2e 잔여다. 대역이 무작위 날짜를 뽑는 구조라
잔여가 쌓일수록 `23505 duplicate key` 확률이 올라간다 — 실패한 실행이 행을 더 남기고 그 행이 다음
실행을 더 자주 죽이는 되먹임이다. **e2e 17건 실패가 이 하나로 설명됐다.** `pnpm db:reset`(2026-08-19
사용자 승인)으로 되돌린 뒤 같은 실행이 89/90으로 올라섰다.

자식 테이블 여섯(`applications`·`assignments`·`ceremonies`·`schedule_position_requirements`·
`assignment_trainees`·`scheduling_audit_logs`)이 전부 `NO ACTION`이라 잔여를 골라 지우려면
의존 순서를 손으로 밟아야 한다. **e2e가 자기 뒤를 못 치우는 것이 진짜 결함이고**, 실행이 중단될 때마다
쌓인다. 정리 책임을 어디에 둘지는 전체 E2E를 CI로 옮기는 P0-T62가 함께 볼 자리다.

**셋째, 잔여를 걷고 나서 남은 진짜 결함 하나.** `recruitment-manage.spec.ts:166`이 `--repeat-each=3`에서
1회 실패한다. 마감일 연장 성공 토스트가 뜨고 다이얼로그가 닫힌 뒤 다시 열면 **옛 마감일이 남아 있다** —
`useRecruitmentManage.submit()`이 `startTransition` 안에서 서버 액션이 끝나자마자 `showSnackbar`·
`close()`를 부르고 목록 재검증이 커밋되기를 기다리지 않는다. 받은 값이 늘 갱신 전 값이라 경합이
아니라 순서 문제다. **자리가 `features/recruitment`라 이 task 밖이고** backlog에 올렸다.



### P0-T59 ID 충돌 — 양보하고 P0-T64로 새로 등록 (2026-08-19 사용자 결정)

다른 세션이 커밋되지 않은 작업 트리에서 P0-T59를 「세그먼트 재편과 데이터 접근 deep module」로 등록했다. 이 task의 문서 여덟 자리는 같은 ID를 「시안 사다리 대조 검사기」로 쓴다 — 그중 하나가 **봉인된 RADIO revision 9의 행**이라 되돌려 고칠 수 없다. 제안은 봉인 이력을 그대로 두고 handoff에 정정 한 줄을 남긴 뒤 사다리 검사기를 **P0-T63**으로 새로 등록하는 것이다. 새 task 등록이라 기획 승인이 먼저다.

두 세션이 부딪히는 것은 ID뿐이다. 상대 쪽 `home.mock.ts` `ui` → `model` 이동은 `depends_on` 사슬이 P0-T48이 닫힌 뒤로 놓여 있어 충돌이 아니다.

**결정.** P0-T59는 상대 세션 것(「역할 세그먼트 재편 시범 이행 — schedule」)으로 두고 시안 사다리 대조 검사기를 새 ID로 옮긴다. 제안 시점의 P0-T63은 답을 받는 사이에 상대가 「역할 세그먼트 구조 계약과 interface 테스트 가드」로 가져가, **번호만 P0-T64로 민다.**

**이 handoff 안의 `P0-T59` 언급 다섯 자리(`:555`·`:626`·`:633`·`:683`·`:691`)와 봉인된 `P0-T48-radio.md` revision 9의 행은 고치지 않는다.** 기록은 그때 무엇을 그렇게 불렀는지를 남기는 것이고, 봉인 이력을 되돌려 고치지 않는 것이 이 저장소의 규칙이다. 읽는 사람은 이 절을 정정으로 본다 — **거기 적힌 P0-T59는 전부 P0-T64를 가리킨다.**

**등록 시점은 P0-T48이 `done`이 된 직후다.** 지금 `00-foundation.md`에 P0-T64 절을 더하면 RADIO가 그 파일에 건 용도 한정(「P0-T48 절의 `test_mode` 정정과 완료 기록」)을 벗어난다. 그 한 줄 때문에 revision 16을 봉인하는 것은 이 task의 지연을 만든 바로 그 되풀이라, `in_progress`가 비어 `gate:scope`가 아무것도 안 막는 순간으로 미룬다. 넘길 범위 셋은 `:626` 절이 이미 적어 뒀다.

### 남은 문서 정정 둘

- **인수 조건 34의 문장** — 「임의 간격 0건」이라고 적었지만 실제로는 `env(safe-area-inset-*)` 셋과 `backdrop-blur-[9px]` 하나가 남아 있고 린트가 이를 허용한다. 값이 없어서가 아니라 임의값이 **필요한 자리**라서 남은 것이다. 조문을 「린트가 허용하는 넷 말고는 0건」으로 좁히는 것이 사실에 맞는다.
- **인수 조건 5의 「쉰 행 전부 닫힘」** — 47행이 닫혔고 2행은 P0-T60으로, 1행은 P0-T55로 갔다. 「전부」가 아니라 「이관분을 뺀 전부」다.
...
**③ worker 마무리 명령이 CI가 실제로 도는 것과 같은 규칙을 보게 한다.** 인수 조건 31을 고치다
드러났다. worker들은 마칠 때 `pnpm lint`를 돌렸는데 CI는 `pnpm lint:ci`를 돌린다. 타입 인식 규칙
(`no-unsafe-assignment`·`no-floating-promises` 부류)이 **CI 설정에만 켜져 있어서**, 통과했다고 보고된
라운드 여럿을 지나 오류 일곱이 쌓인 채로 최종 verify에서 한꺼번에 터졌다. 각 라운드에서 한 건씩
잡혔을 것을 마지막에 몰아서 고쳤다. worker 정의의 마무리 명령을 CI와 같은 것으로 맞추거나, 두
설정이 갈리지 않게 하나로 합친다. 같은 뿌리의 둘째 함정도 함께 적어 둔다 — `pnpm verify 2>&1 | tail`은
**파이프가 종료 코드를 삼켜** 실패를 성공으로 보고한다. 이 저장소에서 두 번째다.
