...
- 시간·easing·spring 상수의 정본은 `shared/config`의 모션 토큰이며 화면에서 값을 직접 지정하지 않는다.
- 다이얼로그·버튼·칩·스케줄 행처럼 지금 반응이 없는 요소에 진입과 눌림 피드백을 넣는다.
- `vaul` 바텀시트와 `sonner` 스낵바는 교체하지 않고 타이밍만 토큰에 맞춘다.
- 하단 탭 이동과 스케줄 목록에서 상세로 들어가는 전환을 넣는다. 전환은 React `<ViewTransition>`을 쓰고 미지원 브라우저에서는 애니메이션 없이 정상 동작한다.
...
- 모션 토큰이 `shared/config`에 있고 `shared/ui`의 움직이는 컴포넌트가 하드코딩 값 대신 토큰을 참조한다.
- `prefers-reduced-motion: reduce`에서 이동·확대 애니메이션이 제거되는 것을 자동 테스트로 확인한다.
- `transform`·`opacity` 외의 속성을 애니메이션하면 lint가 막는다.
- 하단 탭 이동과 스케줄 상세 진입에서 전환이 동작하며, View Transitions 미지원 환경에서도 화면이 정상 렌더된다.
...
- 새 모션 토큰 신설. 필요해지면 P0-T43 범위로 되돌린다.

인수 조건:

- 네 효과가 각 화면에서 동작하고 reduced-motion에서는 즉시 표시나 opacity 변화로 대체된다.
- 스케줄 목록 렌더 시간이 승인된 기준선을 넘지 않는 것을 Playwright로 확인한다.
