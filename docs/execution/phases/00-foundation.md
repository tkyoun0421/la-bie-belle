...
- 하단 탭 이동과 일정 달력의 셀에서 상세로 들어가는 전환을 넣는다. 전환은 React `<ViewTransition>`을 쓰고 미지원 브라우저에서는 애니메이션 없이 정상 동작한다.
...
- 하단 탭 이동과 달력 셀에서의 스케줄 상세 진입에서 전환이 동작하며, View Transitions 미지원 환경에서도 화면이 정상 렌더된다.
- 정적 청크 gzip 합계가 승인된 상한을 넘지 않는 것을 CI에서 확인한다. 2026-08-09 기준선은 367KB다.
...
- 정적 청크 gzip 합계가 400KB를 넘지 않는다. `motion` 도입에 맞춰 P0-T43의 380KB에서 올린 값이며 근거는 ADR-0015다.
