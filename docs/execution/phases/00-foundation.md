...
- 기존 정적 HTML 생성 구조를 유지하면서, 대시보드를 프로젝트 운영의 읽기 전용 판단 진입점으로 확장한다.
- 상단에 기준 시각·기준 커밋, 현재 상태와 단일 다음 행동을 표시한다. 다음 행동은 상태·승인·의존성의 실행 경로를 우선하며 task를 자동 선택·승인·실행하지 않는다.
- 추천 근거, 직접 선행 조건, 차단 사유와 관련 task·문서를 제공하고, 대안 후보와 phase별 진행을 함께 표시한다. 상세 의존성 경로는 필요할 때만 펼쳐 본다.
- task `done`·`blocked`·`skipped` 전환과 phase 경계 상태 변화 뒤에 readiness 보고서와 dashboard 데이터를 갱신한다. 생성 시각·기준 커밋을 표시하고 원본 누락 또는 오래됨은 명시적으로 경고한다.
...
- 대시보드는 task index나 실행 상태를 변경하지 않으며, 사용자 명시 없이 다음 task를 실행할 수 없다.
