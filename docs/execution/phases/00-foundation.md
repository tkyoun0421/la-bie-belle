...
### P0-T48. 전역 디자인 틀 재제안과 기존 화면 퍼블리싱

목표: 앱 전체가 공유하는 틀을 다시 제안해 하나로 확정하고, 이미 살아 있는 화면을 그 틀로 다시 퍼블리싱한다. 디자인 확정과 UI 구현을 한 흐름에 둔다.

- 앱 셸, 카드 구조, 여백 리듬, 타이포 위계를 담은 템플릿을 여러 안으로 제안하고 하나를 확정한다.
- 확정한 틀을 DESIGN 문서에 고정한다. 이후 모든 화면 시안은 그 틀 안에서 나오고, 화면마다 정하는 것은 세부뿐이다.
- 그 틀로 preview에 등록된 기존 화면을 다시 퍼블리싱한다 — 홈, 일정, 확정 상세, 예상 급여, 알림함, 전체, 공통 상태 화면.

비목표:

- 아직 만들지 않은 화면. 알림·교대·출퇴근·급여 화면은 각자의 기능 task 안에서 publisher → implementer 순차로 처리한다.
- 화면 동작·로직 변경. 모양만 바꾼다.
- 서버·데이터 변경.

인수 조건:

- 확정한 틀이 DESIGN 문서에 고정되고, 탈락안과 고른 이유가 `docs/execution/runs/P0-T48/design/`에 남는다.
- preview에 등록된 기존 화면이 모두 새 틀로 렌더된다.
- 토큰 값을 바꿨다면 영향받은 화면의 전후 비교가 기록된다.

주요 경계 사례:

- 틀이 바뀌면서 화면 사이의 미세한 어긋남이 드러날 수 있다. 이 task 안에서 맞추되, 동작이 걸린 문제는 별도 제안으로 돌린다.
- 이 task가 끝난 뒤 P4-T03부터 기능 개발을 재개한다. 그때까지 P4 큐는 대기한다.
