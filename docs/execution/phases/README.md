...
이 Phase 순서는 인터뷰 출발점이며 그 자체가 실행 큐는 아닙니다. 사용자는 우선순위와 범위를 인터뷰에서 바꿀 수 있습니다. 실제 실행 순서는 두 승인을 마친 `planned` task의 의존성 순서이며, 개발 루프는 [연속 루프 규칙](../../workflow/WORKFLOW.md#연속-루프-규칙)에 따라 `planned` 큐가 빌 때까지 이어서 처리합니다. 저장소 전체에서 `kind: task`인 `in_progress`는 언제나 최대 하나입니다.

## 상태 정의

- `proposed`: 기존 기준안 또는 새 아이디어. 기획 승인 전에는 실행할 수 없음.
- `design_pending`: 기획은 승인됐지만 task별 RADIO 개발 설계가 승인되지 않아 실행할 수 없음.
- `planned`: 기획과 RADIO가 모두 승인되고 상세 인수 조건과 실행 계약이 기록된 구현 대기 작업. 개발 루프의 실행 큐.
- `in_progress`: 현재 구현 중. `kind: task` 레코드 중 최대 하나만 허용.
- `verification_pending`: 구현은 끝났으나 등록된 검증이 아직 완료되지 않음.
- `blocked`: 실행 중 새 결정이 필요하거나 기술적 실패가 재시도 한도를 넘겨, 작업물과 증거를 보존한 채 안전 중단됨. 루프는 종료하지 않고 이 task에 의존하지 않는 다음 후보로 진행합니다.
...
5. `depends_on`이 모두 `done`인 `planned` task를 의존성 순서로 고른다.
6. 후보 하나만 `in_progress`로 바꾸고 개발 단계에서 승인된 범위와 RADIO 안에서 구현한다.
7. 검증 단계에서 task에 적힌 verification과 관련 회귀 테스트를 실행한다.
8. 리팩토링 단계에서 동작을 바꾸지 않고 구조만 정리한 뒤 같은 검증으로 GREEN을 다시 확인한다.
9. 인수 조건을 충족한 증거에 관련 spec ID를 남기고 `done`으로 갱신한다.
10. handoff를 기록하고 다음 후보 task로 이어간다. `planned` 큐가 비면 루프를 종료하고 전체 결과를 일괄 보고한다.

모든 task는 `approval_contract`를 명시한다. 전환 전에 종료된 이력만 `legacy-v2`이며 실행 상태로 돌아갈 수 없다. 현재 미완료와 신규 task는 `dual-approval-v3`를 사용한다. 실행 중 새 결정이나 재시도 한도 초과는 먼저 `blocked`와 구조화된 결정 신호로 보존하고, 루프는 그 task에 의존하지 않는 다음 후보로 계속 진행한다. 인터뷰 확인 후 제품 변경은 `proposed`, 기술 변경은 `design_pending`으로 선택적으로 반환한다.
...
JSONL은 AI가 전체 문서를 읽기 전에 인터뷰 후보와 승인된 현재 작업을 구분하고 관련 파일을 빠르게 찾는 용도입니다. `spec_refs`는 `PRD:AC-01`, `PRD:INV-ATT-01`, `PRD:ACCT-DORMANT-01`, `DOMAIN:ATTENDANCE`, `ADR:0004`, `DOCS:SDD` 형식을 사용합니다. 상세 인수 조건은 연결된 Phase 문서, 요구사항 원문은 PRD·Domain·ADR과 문서 지도에서 확인합니다.
