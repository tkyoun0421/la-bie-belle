...
| 3 | 2026-08-09 | 확정 스케줄 복사 제외 재봉인(사용자 승인). 교차 검증 확정 발견이 드러낸 결과 모순 — revision 2의 인수 조건 2가 복사의 거부 대상을 CANCELLED로만 적어 CONFIRMED 스케줄도 준비 화면 진입만으로 표가 생성되는데, 그 표는 수정 함수가 CONFIRMED를 거부해 P3-T06 전까지 고칠 수 없고 확정 후 추가된 포지션까지 포함된다(PRD 7장 "확정 스케줄은 자동으로 바뀌지 않되"와 결과 상충). 복사 거부 대상에 CONFIRMED를 추가한다. |
...
- **복사는 멱등이며 확정·취소 스케줄을 제외한다**: 준비 진입 시 표가 없으면 그 시점 활성 포지션의 기본 인원수로 생성하고, 있으면 아무것도 하지 않는다. 동시 진입은 `on conflict do nothing`으로 한 번만 생성된다. `CONFIRMED`·`CANCELLED` 스케줄은 복사 대상이 아니다 — 수정 함수가 확정을 거부하므로 복사만 허용하면 고칠 수 없는 표가 생기고, 확정 후 추가된 포지션이 섞여 "확정 스케줄은 자동으로 바뀌지 않는다"는 제품 규칙과 어긋난다(revision 3).
...
2. `copy_schedule_requirements(target_schedule_id)` — is_admin(42501) → 대상 존재·`CONFIRMED`·`CANCELLED` 거부(revision 3) → 표 존재 시 무변경 → 활성 포지션 기본값 일괄 생성 → 감사 1행. 재호출 무변경·감사 0행 — pgTAP.
...
| 2 복사 | 테스트함 — 활성 포지션 전부 생성 | 테스트함 — CONFIRMED·CANCELLED·미존재 거부 | 테스트함 — 활성 0개면 빈 표 | 테스트함 — 비관리자 42501 | 테스트함 — 재호출 무변경 | 테스트함 — on conflict 단일 생성 |
