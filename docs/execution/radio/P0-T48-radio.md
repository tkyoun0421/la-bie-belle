...
| revision | 날짜 | 내용 |
| --- | --- | --- |
| 15 | 2026-08-19 | **`pnpm verify` 전체를 처음 돌려 나온 둘을 연다**(사용자 결정). ① `pnpm lint:ci`가 `eslint.config.ci.mjs`로 타입 인식 규칙을 켜는데 라운드마다 worker가 돌린 것은 `pnpm lint`뿐이라 7건이 쌓여 있었다 — 전부 이 task가 새로 만든 파일이다(`block-boundary.tsx`의 떠 있는 promise 하나, 테스트 셋의 `any` 누출 여섯). 고치는 것을 연다. ② `recruitment-flow.spec.ts:304-305`가 홈에서 「근무 신청 마감이 임박했어요」와 「지금 신청하기」를 기대하는데, **그 표시는 확정 설계가 홈에서 걷은 것이다** — `NOTES.md:231` 「마감 임박 표시를 홈에서 잃는 대신 알림 한 장이 그 역할을 진다(2026-08-17 사용자 확정)」이고 알림 카드 종류 다섯에도 마감 임박이 없다. 인수 조건 38이 `selectImminentRecruitment`를 지웠고 revision 10이 홈을 목 데이터로 돌린 것이 그 확정의 코드 쪽 귀결이다. 그러므로 이 단언 둘은 폐기된 설계를 지키는 낡은 테스트이고, **걷되 알림 화면에서 다시 세우는 일을 backlog에 올린다**(화면은 P0-T51). 테스트 이름이 말하는 나머지 둘 — 달력 월 경계와 마감 표시 — 은 그대로 둔다. 2026-08-19 사용자 결정. |
...
src/shared/model/__tests__/route-transition.test.ts
tests/e2e/**
src/app/layout.tsx
src/widgets/offline/**
src/views/home/**
...
**인수 조건 31의 잔여 둘(revision 15).** `src/shared/ui/block-boundary.tsx`는 **떠 있는 promise 한 줄에 `void`를 붙이는 데** 한정한다 — 재시도 흐름을 바꾸지 않는다. `src/shared/ui/__tests__/block-boundary.test.tsx`·`src/app/(protected)/__tests__/providers.test.tsx`·`src/shared/hooks/__tests__/useAmountMasking.test.ts`는 **`any` 누출을 막는 타입 주석에** 한정하고 단언을 고치지 않는다. `tests/e2e/recruitment-flow.spec.ts`는 **홈 단언 둘(`근무 신청 마감이 임박했어요`·`지금 신청하기`)을 걷는 데만** 한정한다 — 같은 테스트의 달력 월 경계·마감 표시 단언과 시딩은 그대로 둔다.
