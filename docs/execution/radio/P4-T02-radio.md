...
- 적용 깊이: 중간 — push_subscriptions 쓰기 RPC 2종 신설, Service Worker 엔트리 1개, 새 화면 1개(알림 설정)와 시트 1개(사전 안내), 제품 서버 로직 변경 없음(발송은 T08).
- test mode: tdd
...
- 코드 대조 확정 사실: PWA manifest(`src/app/manifest.ts`)·아이콘·env는 P0에 준비돼 있다 — `.env.example`에 `NEXT_PUBLIC_VAPID_PUBLIC_KEY`·`VAPID_PRIVATE_KEY`·`VAPID_SUBJECT`가 있고 `src/shared/model/env.ts`가 public 키를 필수로 검증하며 `env.client.ts`가 이미 노출한다 — env 스키마 변경 없음. Service Worker는 없다. Next.js 16 가이드(`node_modules/next/dist/docs/01-app/02-guides/progressive-web-apps.md`)의 관례는 `navigator.serviceWorker.register(new URL(<워커 파일>, import.meta.url), { scope: "/", updateViaCache: "none" })` 번들 방식 — `next.config` headers 변경 불필요. `push_subscriptions` 스키마(endpoint unique, p256dh·auth, disabled_at, 본인 select RLS만)는 P4-T01이 만들었고 쓰기 경로가 없다. 더보기 메뉴는 `src/views/more/ui/MoreView.tsx`의 정적 목록이고, 신청 저장 흐름의 호스트는 `src/views/schedule/ui/ScheduleView.tsx`(`useApplicationBatch`)다. `shared/ui`에 토글 컴포넌트는 없다 — 기존 `button`·`bottom-sheet`로 충분해 신설하지 않는다. e2e 밴드 마지막 사용은 528~559(notifications). fsd 대조(2026-08-16): `config/fsd.json`의 appLayer는 unitTest required(exemptFiles는 page·layout 계열뿐)이고 lib·model·hooks·api 세그먼트도 required, ui는 exempt — SW 파일을 어디 두든 단위 테스트가 따른다. 기존 단언 대조(2026-08-16): `tests/e2e/tab-navigation.spec.ts`에는 더보기 메뉴 목록 단언이 없고, `src/views/more/ui/__tests__/MoreView.test.tsx`가 링크 개수를 고정 단언한다(`toHaveLength(2)`) — 메뉴 항목 추가 시 이 테스트의 정합 갱신이 필요하다.

## Requirements

### 범위와 비목표

범위: 마이그레이션 1개(구독 저장·해지 RPC 2종), Service Worker 엔트리 + push 표시·클릭 이동의 순수 함수 분리, `features/push`(actions·훅·판정 model·사전 안내 시트), 알림 설정 화면(`/more/notification-settings`)과 더보기 메뉴 항목 1개, ScheduleView 사전 안내 삽입, dev 발송 스크립트(`scripts/send-test-push.mjs`, web-push devDependency), 실기기 체크리스트 문서, pgTAP 25번, e2e 1 spec(밴드 561~592).
...
- 푸시 페이로드 해석(제목·본문·target)과 클릭 이동 경로 계산은 순수 함수로 두고 SW 파일은 이벤트 배선만 한다. target 모양은 P4-T01 `NotificationTarget`과 동일하다.
...
- fsd 세그먼트 규칙이 SW 엔트리 배치(`features/push/lib`)와 충돌해 `config/fsd.json` 변경이 필요해지는 경우.
...
8. SW 순수 함수: push 페이로드 → `showNotification` 옵션(제목·본문·아이콘·data.target), `notificationclick` target → 이동 경로(`/schedule/<date>`·`/pay`, 알 수 없는 값은 `/notifications` 폴백)(단위 — RED 먼저). SW 배선 파일은 `self` 스텁으로 push·notificationclick 리스너 등록과 model 위임을 단언한다(lib 세그먼트 unitTest required).
...
| 8 SW 함수 | 테스트함 — 페이로드→옵션·target→경로 | 테스트함 — 알 수 없는 target 폴백 | 테스트함 — 페이로드 누락 필드 | 해당 없음 — 표시 계층 | 해당 없음 — 순수 함수 | 해당 없음 — 순수 함수 |
...
- `src/features/push/lib/push-service-worker.ts` — SW 엔트리(배선만): `push` → `model/push-display.ts`의 옵션 변환 호출 후 `showNotification`, `notificationclick` → 같은 model의 경로 계산 후 `clients.openWindow`/focus. 훅이 같은 슬라이스 상대 경로로 `navigator.serviceWorker.register(new URL("../lib/push-service-worker.ts", import.meta.url), { scope: "/", updateViaCache: "none" })` 번들 등록한다(Next.js 16 가이드 관례) — app 계층 파일·상향 참조 없음.
- `src/features/push/` — `api/save-push-subscription.ts`·`api/remove-push-subscription.ts`(server action, RPC 호출, 기존 오류 매핑, 단위 필수), `hooks/usePushSubscription.ts`(SW lazy 등록·권한 요청·subscribe/unsubscribe·상태), `model/push-support.ts`(상태 판정 순수 함수)·`model/push-priming.ts`(노출 판정)·`model/push-display.ts`(SW 순수 함수), `ui/PushPrimingSheet.tsx`(bottom-sheet 재사용).
...
- 용도 한정: `src/views/more/ui/**`는 메뉴 항목 1개 추가와 `MoreView.test.tsx` 링크 개수 단언 정합 갱신(2→3)에만, `ScheduleView.tsx`는 사전 안내 시트 삽입 한 곳, `package.json`·`pnpm-lock.yaml`은 web-push devDependency 1개, `tests/e2e/support/**`는 밴드 1개 추가(561~592)와 헬퍼 재사용에만 쓴다.
...
- 알림 클릭 딥링크의 알림함 밖 진입(푸시 → 상세 직행) 계약은 이 task의 push-display 함수가 소유하고, T03·T08이 재사용한다.
