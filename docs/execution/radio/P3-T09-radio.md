...
| 3 | 2026-08-15 | 개발 단계 정지 조건 반환의 해소(2차). 승인된 교육 칩 숨김이 `tests/e2e/assignment-trainee.spec.ts`의 direction2 시나리오와 충돌한다 — 다른 포지션 정식 배정자의 「교육」 버튼 클릭으로 DB 거부(LB024)를 확인하던 경로인데, 칩 숨김으로 그 버튼 자체가 사라져 클릭이 불가능하다. 이 파일을 허용 경로에 추가하고 용도를 direction2 시나리오만 UI 클릭 대신 direct RPC 호출로 LB024 거부를 확인하도록 재정렬하는 것으로 한정한다(같은 파일의 기존 direct RPC 패턴 재사용, DB 거부 단언 보존). 2026-08-15 사용자 결정. |
...
- 용도 한정: `src/features/{assignment,ceremony,requirement}/**`는 새 오류 코드 매핑과 그 단위 테스트에만 쓰고 기존 입력·응답 계약을 바꾸지 않는다. `error-codes.config.ts`는 코드 3개 추가뿐이다. `supabase/tests/**`는 새 파일(23), roster 확장 키 단언 갱신(22), 그리고 기존 파일(08~21)의 「CONFIRMED 거부」 단언을 이번 개방에 맞춰 갱신하는 데만 쓴다 — 갱신 목록은 runs/radio.md에 남긴다. 그 밖의 기존 단언 약화는 금지다. 기존 e2e 3파일(ceremony-edit·schedule-confirmation·assignment-eligibility)은 확정 후 읽기 전용 단언을 새 동작(편집 가능·취소 버튼)에 맞춰 갱신하는 데만 쓴다(revision 2) — 다른 시나리오·단언은 건드리지 않는다. `tests/e2e/assignment-trainee.spec.ts`는 direction2 시나리오만 UI 클릭 대신 direct RPC 호출로 LB024 거부를 확인하도록 재정렬하는 데 쓴다(revision 3) — DB 거부 단언은 보존하고 다른 시나리오·단언은 건드리지 않는다. `tests/e2e/support/**`는 밴드 1개 추가와 기존 헬퍼 재사용에만 쓴다.
