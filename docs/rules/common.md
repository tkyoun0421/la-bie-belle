...
어느 언어로 쓸지는 여기서 정하고, 그 언어를 어떻게 쓸지는 `docs/rules/matchers/writing-korean.md`가 정한다. 이 저장소의 글은 대부분 LLM 세션이 쓰므로 번역투와 상투구가 그냥 두면 쌓이고, 규칙 문서에 쌓인 문체는 다음 세션이 본으로 삼는다.
