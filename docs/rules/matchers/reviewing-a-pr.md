...
status: "active"
related_adr: ""
related_issue: "#69, #89, #101, #114, #113, #91, #126, #130, #137"
---
...
6. **완료 증거** (high) — 코드가 바뀐 PR인데 원장이 아예 없으면 그 자체가 발견이다. 훅은 원장이 없으면 통과시키므로 이 자리가 그 구멍을 메운다. 원장이 있으면 PR 본문에 전문이 실렸는가, 미충족 gate가 남았는가, 포기에 사유가 붙었는가. 그리고 `CHECK:`가 gate 제목이 말하는 것을 실제로 재는가. 마지막 항목이 이 자리의 요점이다 — 검사는 명령이 초록인지만 알지 한국어 제목과 셸 코드가 같은 것을 말하는지는 모른다. `G1: 청구서가 맞아떨어진다`에 `CHECK: node -e "console.log('ok')"`가 붙어도 검사는 통과시킨다. `docs/rules/matchers/gates.md`를 봐라.
...
7번과 8번은 기계로 옮기지 않는다. 그 둘의 절반을 CI로 내리자는 안이 #91에 있었는데, 세어 보니 남는 절반이 더 컸다. 제목 형식은 정규식이 잡지만 Impact 줄이 실제로 무엇이 달라지는지 말하는가는 못 잡고, 문체 항목은 인용문과 코드 블록 안에서도 똑같이 걸려 오탐이 merge를 막는다. 축을 반으로 가르면 리뷰어는 남은 반을 보러 같은 파일을 다시 연다. front matter가 #126에서 CI로 간 것은 그 갈림이 아니다. 기계가 받은 것은 형태라는 축 하나의 전부이고, 7번에 남은 것은 값이라는 다른 축이다. 값은 리뷰어가 diff를 읽으며 어차피 지나는 자리라 같은 파일을 다시 여는 걸음이 없다.
