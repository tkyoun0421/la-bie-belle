...
담당은 경로가 정한다. `[Ticket]`과 `[Request]`는 본문에 적은 경로를 `config/ownership.json`에 대면 소유 역할이 나오고, pm·dev·ui 어느 목록에도 없으면 총괄이다. 폼에서 담당을 고르는 칸을 뺀 이유가 이것이다 — 손으로 고르게 두면 경로와 어긋나고, 어긋난 쪽은 언제나 손으로 고른 쪽이다. `[Slice]`는 언제나 Dev다. `[Epic]`만 경로가 없어 registry가 답하지 못하므로 폼의 담당 칸에서 고른다.
