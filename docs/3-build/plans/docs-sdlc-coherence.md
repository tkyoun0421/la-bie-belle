...
- **연락처 수정** — [api/account.md](../../2-design/modules/account/design.md#행위별-구현-계약)가 정본이다. `profile_private` 본인 행 직접 갱신이고 함수가 아니다. [runtime/account.md](../../2-design/modules/account/design.md#프로필-제출연락처사진)의 「이름·연락처 변경(`set_display_name`·`submit_profile`)」은 「이름 변경(`set_display_name`)과 연락처 변경(`profile_private` 직접 갱신)」으로 고친다. 낙관적 처리는 그대로다.
