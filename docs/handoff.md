...
**schedule-admin 시안을 태관이 실제로 열어 보고 「제안」들을 판정하는 것이다.** 관리자 근무표 제작·확정(⑥)의 인터뷰(결정 13건)와 디자인(`pages/schedule-admin.md` + 시안, 화면 아홉)이 끝났다. 인터뷰가 도메인 미정 둘을 해소했고(`schedule.md` 확정 뒤 날 열기, `notification.md` 강제 변경 통지 확인) 마감일 변경 알림이 일곱째로 늘었다. 문서의 「아직 안 정한 것」 15건과 「제안」 표시들이 판정 대상이다 — 확정 뒤 교육 붙이기와 확정 뒤 새로 연 날의 자리 조정 둘은 도메인 결정이라 무게가 더 있다.
