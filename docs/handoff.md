...
**관리자 근무표 제작·확정(⑥) 화면의 인터뷰와 디자인이다.** 태관이 2026-09-07에 그리는 순서를 관리자 근무표부터로 골랐다 — 제일 크고 복잡한 화면을 먼저 뚫어 근무표 구조를 확정하면 근무자의 근무표 보기가 따라온다. 순서 규칙(디자인 먼저, 관리자 근무표부터)은 `docs/1-plan/roadmap.md`「순서 규칙」에 박았다. 나머지 화면의 순서는 그리면서 정한다.

산출물은 `docs/2-design/design-system/pages/`의 페이지 문서와 `.sian.html` 시안이다. `pages/dashboard.md`의 절 구성(화면마다 짜임 → 색 → 글자 → 여백과 모양 → 문안 → 모션, 끝에 「규칙과 부딪힌 자리」와 「아직 안 정한 것」)을 따른다. 빈 상태·실패·상한·잠금·재진입까지 전부 그리고, 미정은 「제안」으로 밝힌다. 시나리오 장면 2(관리자 윤서준, 휴무 희망 보며 근무표 짜고 확정)가 이 화면의 이야기고, 1차 판정 기준에 들어 있다.

인터뷰에서 같이 정할 도메인 미정: 확정 뒤 날 열기(`schedule.md`「아직 안 정한 것」의 유일 항목). 확정 알림 쪽을 스치면 금요일 확정 겹침·강제 변경 통지(notification)도 걸릴 수 있다.
