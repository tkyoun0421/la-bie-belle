...
**schedule-admin 시안 2차 검수가 2026-09-07에 끝났다.** 태관이 비교 시안을 보고 둘을 정했다 — 달력 열린 날 칸은 카드(흰 면+그림자+테두리) 대신 `bg.neutral-weak` 면 채움, 확정 뒤 빈 자리 표시는 글자 「빈 1」 대신 점선 원 + 수. 정본 문서와 시안에 반영됐고, components.md 근무표 날짜 칸 표의 카드 모양(근무 있음·교육 배정)도 같은 판단이 필요하다는 메모가 「규칙과 부딪힌 자리」에 붙었다 — 근무자 ①근무표 보기를 그릴 때 정한다. 남은 「제안」은 하나다 — 관리자 푸시 시점(예식 3일 전 저녁 9시).

다음은 남은 화면의 순서다 — 「그리면서 정한다」로 열어뒀다. 근무자 ①근무표 보기가 ⑥의 구조를 그대로 받으니 자연스러운 다음 후보다.
