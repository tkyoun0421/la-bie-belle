...
**다음 첫 수는 `sian-toss-pass`다 — 대시보드 라이트·다크 두 장을 먼저 그려 승인받고 나머지 열다섯을 전개한다.** 인터뷰(2026-09-25)가 룩앤필 기준점을 토스 홈으로 옮겼고 [ADR-014](2-design/adr/ADR-014-toss-like-depth-and-graphics.md)가 ADR-012의 마감 넷을 뒤집었다 — 회색 바닥 위 흰 카드, `shadow.card` 하나(다크 `none`), 밀도 회복(눈금 20), 36px 큰 숫자 하나(검정), 토스페이스 SVG + AI 3D, 움직이는 스켈레톤, 다크 1차. `design-system-toss`(#411)와 `illustration-style-guide`(#410)가 `done`이라 디자인 시스템 정본과 [illustration.md](2-design/design-system/illustration.md)가 섰다. 시안은 화면마다 라이트·다크 둘, 큰 글자(1.3배) 하나, 관리자 달력·날 상세는 태블릿 가로 한 장 더다. 새로 서는 절은 QR 화면의 「홀 위치」, 대시보드·급여의 36px 숫자, 스켈레톤 목업이고 `sian-sync`의 어긋남 목록을 같이 닫는다. [admin-home.md](2-design/system/screens/admin-home.md)에 「관리자」 스위치 이전 서술이 남아 있어(뒤로 가기·탭 바 줄) 시안을 그리기 전에 문서를 먼저 맞춘다. 그 뒤 순서는 `ui-kit`(조각 라이브러리 + `/_catalog` + `pnpm tossface:fetch`, 랄프 루프 첫 대상) → 화면 루프다.
