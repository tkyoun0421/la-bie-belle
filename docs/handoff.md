...
**근무자 ①근무표 보기가 2026-09-07에 검수까지 끝나 merge됐다(#262).** 문서 `pages/schedule-worker.md` + 시안(시나리오 보기 첫 도입) + 도메인 개정(근무 취소 다섯째·교대 다중 요청과 관리자 선택 승인·알림 아홉째) + components.md 날짜 칸 개정(면 채움·교육 점 구분 걷음·관리자 상태 합류)이 한 PR로 들어갔다. 검수 승인으로 보낸 뒤 흐름(「요청 중」 배지·버튼 잠금), 아코디언 기본 접힘, 취소 마감(근무 전날), 사유 상한 100자까지 확정됐다. **②휴무 희망 제출 인터뷰가 다음 첫 수다** — 이 화면의 미정 「확정 전·근무표 없는 달의 모습」과 얽혀 같이 정한다.

이번 회차의 프로세스 결정 셋 — ⑴ 시안에 시나리오 보기(장면 순서로 한 장씩 넘김)를 넣는다, 다음 시안에도 이어지면 sian-writer 정의문으로 추출. ⑵ sian-writer 모델이 opus에서 sonnet으로(#259 뒤집음) — 반려가 늘면 되돌린다. 첫 실전(엣지 절 추가)은 문제 없었다. ⑶ 정본 문서에 결정 주체·날짜 괄호를 안 적는다 — 이력은 커밋 몫. design-system README에는 「해피 패스는 끝까지 그린다」 조항이 새로 섰다.
