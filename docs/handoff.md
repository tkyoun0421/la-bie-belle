...
**받는 것의 방향이 뒤집혔다 — 「휴무 희망」이 「근무 신청」이 됐다.** 일할 수 있는 날을 신청하고 신청 안 한 날이 곧 휴무다. 미신청 날에 사람을 넣는 길은 「근무 요청」(관리자가 자리 단위로 물음 → 수락 = 즉시 배정, 여럿 동시·선착순, 48시간 만료) 하나뿐이고 강제 배정은 없다. 별도 제출 화면(②)은 사라졌다 — 근무자 근무표 달력이 확정 전에는 곧 제출 모드다(고르고 보내기, 마감 전 덮어쓰기 재제출, 상한 없음). 접수 열림은 전원 알림. 이 인터뷰가 schedule-worker의 미정 넷(확정 전 모습·과거 달 무제한·대시보드 연결·교대 부제)도 같이 닫았다. 정본 개정은 domain 둘(schedule·notification)·화면 문서 둘·prd·scenarios·intent·writing.md·색인들에 들어갔다.
...
시안 갱신이 다음 첫 수다 — 근무 신청 인터뷰(#265 예정)로 문서가 시안보다 앞섰다. `schedule-worker.sian.html`에 「확정 전 — 근무 신청」 절(제출 모드 달력·마감 뒤·안 만든 달)이 통째로 없고, `schedule-admin.sian.html`에는 옛 「휴무 희망」 문안이 남아 있다. sian-writer를 두 파일에 돌리고 검수 라운드를 거친다.

그다음은 근무 요청 화면이다 — 관리자가 요청을 보내는 문과 요청 상태 목록(schedule-admin 미정), 근무자의 수락·거절 화면(schedule-worker 미정). 두 문서의 「아직 안 정한 것」에 있다.

관리자 쪽 파장 — 교대 수락자 중 고르기와 근무 취소 승인이 관리자 화면 어딘가에 서야 한다. ⑦사유 승인·거절을 그릴 때 같이 본다. 교대 요청 받는 쪽 화면(수락·거절)도 화면 목록에 새로 필요하다. 남은 「제안」: 관리자 푸시 시점(예식 3일 전 저녁 9시), schedule-worker의 「제안」 일곱(고른 날 칸·BottomCTA·0개 제출·토스트·안내 문구 둘·교대 부제) — 시안 검수에서 같이 본다.

남은 화면 목록(⑥ 뒤 순서 미정):

- 근무자: ③급여 조회(주/월/연, 확정 없이 재계산되는 예상치) ④프로필(이름 입력·수정, 승인 전에도 열림) — ①근무표 보기는 봉인, ②는 별도 화면이 아니게 됐다(①의 확정 전 모드)
- 근무자 파생: 교대 요청 받는 쪽 화면(수락·거절)과 근무 요청 수락 화면이 목록에 없다 — 절차가 확정돼 필요해졌다. 순서 정할 때 같이 본다
