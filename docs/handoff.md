...
**1차 릴리스 전체 화면의 디자인을 먼저 그리고 구현은 그 뒤다.** 태관이 2026-09-07에 방향을 바꿨고("계획과 화면 디자인을 먼저, 구현은 나중에, 계획과 설계가 꼼꼼히"), 묶음 범위도 같은 날 "1차 전체"로 골랐다. backlog 순서(대시보드 구현 착수)가 아니다 — 대시보드 spec 자체가 데이터 task 선행을 요구해 지금 못 연다.

화면 목록은 도메인 정본(schedule·attendance·payroll·account·notification)에서 이미 뽑아뒀다. 설계가 끝난 것은 로그인·승인 대기(구현까지 완료)와 근무자 대시보드·출근 인증·사유 시트(`pages/dashboard.md` + 시안, 구현 대기)뿐이고, 백지는 다음 열이다.

- 근무자: ①근무표 보기(주 단위·월요일 시작·달 경계 넘음) ②휴무 희망 제출(근무표 생성 뒤 마감일까지) ③급여 조회(주/월/연, 확정 없이 재계산되는 예상치) ④프로필(이름 입력·수정, 승인 전에도 열림)
- 관리자: ⑤가입 대기 목록(승인·거절·차단, 사진으로 동명이인 구분) ⑥근무표 제작·확정(날 열기·자리 기본 11명·배정·겸임·특수 포지션 자격·확정은 되돌릴 수 없음 — 제일 크고 여러 화면일 수 있다) ⑦사유 승인·거절 ⑧시급 설정·조정 ⑨QR 갱신 ⑩직원 관리(관리자 승격·퇴사)
- 알림은 별도 화면이 없다 — 푸시 + 대시보드 맨 위 안 본 알림(설계 완료)

첫 수는 태관에게 **그리는 순서**를 묻는 것이다. 선택지 셋을 만들어뒀었다: (a) 근무자부터(①→④, 대시보드 설계 자산과 이어짐, 장면 1·3·4·6이 걸림) (b) 관리자 근무표부터(제일 복잡한 ⑥을 먼저 뚫어 근무표 구조를 확정하고 근무자 보기가 따라옴) (c) 시나리오 장면 1→6 순서. 순서 규칙(디자인 먼저)을 어느 문서에 박을지도 안 정했다 — roadmap.md가 후보다.
