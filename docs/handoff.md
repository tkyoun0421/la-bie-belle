...
**랄프 루프가 돌고 있다 — 다음 첫 수는 `profile-form`이다.** 「전체 기능 구현해」(2026-09-27)로 구현 단계에 들어갔다. 루프의 순서와 멈추는 조건은 [spec/ui-kit.md 「루프」](2-design/spec/ui-kit.md#루프)가 정본이다 — ready 맨 위 → `test-planner` → writer → `implementer` → `pr-diff` → 총괄 merge → backlog·handoff. 첫 대상 `ui-kit`이 `done`이다(#414): `src/shared/ui/` 조각 서른셋, `src/app/_catalog.tsx`, eslint 규칙 19(화면 파일의 색·글자·모양 유틸을 막는다), `pnpm tossface:fetch`, Jest가 `logic`·`components` 두 갈래(`jest.projects.js`). **화면 task는 이 조각을 가져다 짜기만 한다.** 시안 열다섯은 그리지 않는다 — 대시보드 두 장 승인으로 시안 전개는 끝났고 화면 task가 ADR-014를 코드에서 세우며 화면 문서(ADR-014 이전 서술)를 같은 PR에서 맞춘다. `dashboard.md` 다시 쓰기(카드 다섯·큰 숫자 하나·앱바 종·마크·「관리자」)는 `dashboard` task 차례에 한다.

**루프가 사람을 부르는 자리 셋.** 실기기 확인(카탈로그 눈 확인, expo-scaffold의 남은 AC, iOS는 개발 빌드가 서야), 3D 석 장(`no-schedule`·`all-clear`·`server-error`), 로컬 Supabase 구글 프로바이더. `ui-kit`이 남긴 사실 — gorhom 바텀시트는 Jest 대역 위에서 죽어 면(`BottomSheet`)과 제스처 컨테이너(`DraggableSheet`)로 갈렸다. SVG에 `var(--color-*)` 문자열을 넘기면 Tailwind가 그 변수를 `:root`에 내놓아 다크가 깨진다 — `SvgPaint` + `fill-*`/`stroke-*` 유틸로 간다(components.md에 적을 값). 자동 리뷰 잡이 코멘트 없이 초록이었다([관찰 019](observations/019-review-job-passes-without-comment.md)).

**디자인 시스템 전개가 남긴 사실 셋.** 팔레트 칸의 `—` 행(라이트·다크가 다른 단계를 가리키는 행)이 여섯이다 — 층 셋과 `bg.neutral-weak-pressed`·`bg.neutral-disabled`·`bg.brand-weak`. **하나 더 늘면 다크 곡선을 다시 짠다**(tokens.md 층 셋 설명). 대비표에서 가장 여유 적은 줄은 `fg.brand` on `bg.brand-weak` 다크 4.96이고, `fg.neutral-subtle`이 안쪽 면 위에서 라이트 4.43으로 떨어지는 것은 components.md 「아직 안 정한 것」에 미정으로 남았다. spec 둘이 새 빈 상태 절(그림 있음)과 어긋난다 — `payroll-adjust.md` 88줄, `schedule-admin.md` 116줄 — 화면 전개 때 고친다.
