...
**관리자 쪽 「제안」 여섯을 검수한다.** 근무자 쪽은 #271에서 문구 축까지 닫혔고 토큰 표에 둘(요청 시트 안내·끝난 요청 안내의 `fg.neutral-muted`·`text-sm`)만 남았다 — 다른 안내 문구와 같은 값이라 판정이 가볍다. 남은 무게는 admin이다: 요청 대기 배지 문구·모양(「요청 2건 대기 중」), 픽커 미신청 줄의 상태 문구와 체크박스 유무, 체크박스 색, 보내기 체크·버튼 모션, 관리자 푸시 시점(예식 3일 전 저녁 9시), 가입 대기 줄. 검수 방식은 스크래치 비교 시안이다 — 후보를 나란히 그려 눈으로 고르고, 결정만 정본에 옮긴다.

새로 열린 미정 하나도 같이 본다 — 달 고르기 시트에서 오늘이 든 달을 표시할지(#273).
