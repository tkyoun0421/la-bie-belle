...
**schedule-admin 시안 검수가 2026-09-07에 끝났다 — PR #260 머지가 첫 수다.** 태관이 시안을 실제로 열어 판정했고 「아직 안 정한 것」 열넷이 전부 닫혔다. 화면 수정(다크 토글, 빈 상태 가운데 덩이, 달력 칸 두 단·사람 아이콘, 스케줄 신청 마감일 용어)과 도메인 판정(확정 뒤 교육 붙이기 허용, 새로 연 날 자유 조정, 만들기의 달 비구속 — 차단 축은 날짜, 찬 자리 합침 허용, 알림 여덟째 — 관리자 빈 자리 재촉)이 문서 넷과 시안에 반영됐다. 남은 「제안」 둘 — 확정 뒤 달력 「빈 1」 모양, 관리자 푸시 시점(예식 3일 전 저녁 9시) — 은 실제 화면을 보고 정한다.

그다음은 남은 화면의 순서다 — 「그리면서 정한다」로 열어뒀다. 근무자 ①근무표 보기가 ⑥의 구조를 그대로 받으니 자연스러운 다음 후보다.
