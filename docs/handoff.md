...
**대시보드 검수를 이어간다 — 규칙 충돌 열하나와 화면 미정 열일곱이 남았다.** 근무표 두 화면은 닫혔지만 `pages/dashboard.md`는 검수를 통째로 안 거쳤다. 이 회차가 그 문서의 「규칙과 부딪힌 자리」 열넷 중 셋을 닫았다 — 띠 막대 토큰(`bg.brand-muted`), 덮개 토큰(`bg.scrim`), 경고 아이콘(닫을 것이 없었다, `tokens.md`가 이미 예외로 적어둔 자리였다).

남은 열하나는 성격이 둘로 갈린다. **규칙 문장 수정 여섯** — BottomCTA 예시에서 출근 인증 빼기(`components.md:53`에 「출근 인증이 여기다」가 아직 있다), `color.md`의 브랜드 자리 넷 목록에 「눌리지 않는 표식」을 더할지, ListRow를 대시보드 알림 줄에 안 쓰는 것, Input 오류 테두리를 안 쓰는 것, 값 없을 때 `–`, 「사유」를 `writing.md` 용어 표에 올리기. **규칙 신설 셋** — 최소 터치 크기(`spacing-shape.md`에 조항 자체가 없다), 하루 띠를 `components.md`에 새 조각으로, 알림의 셋째 상태 「답했음」(`notification.md`에 읽음·안 읽음 둘뿐).

그다음이 화면 미정 열일곱이다 — 지도 SDK 고르기(카카오맵·네이버지도), 홀 좌표를 저장소 밖 어디에 둘지, 위치 권한 거부, 사유 수정·마감·재제출, 카운트다운 시작 시점, 짧은 막대의 시각 배치, 겸임 배정의 막대가 둘인지. 지도 SDK는 `web-researcher`가 붙어야 하고 나머지는 인터뷰다.
