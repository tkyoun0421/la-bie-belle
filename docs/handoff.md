...
**화면 미정이 바닥났다 — 다음 화면으로 간다.** 근무자 근무표와 관리자 근무표 둘 다 「아직 안 정한 것」이 비었고 「제안」 표시도 남지 않았다. 갈래가 둘이다: backlog 「다음」에 홀로 남은 근무자 대시보드 spec, 또는 아래 「남은 화면 목록」에서 하나. 대시보드는 이미 문서(`pages/dashboard.md`)와 시안이 있어 spec만 쓰면 되고, 나머지는 화면 문서부터 시작한다. `roadmap.md`는 근무표 다음 순서를 안 박아뒀다 — 「나머지 화면의 순서는 그리면서 정한다」라 어느 쪽을 먼저 잡을지가 이 회차의 첫 결정이다.
