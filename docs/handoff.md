...
**루프가 사람을 부르는 자리 셋.** 실기기 확인(카탈로그 눈 확인, expo-scaffold의 남은 AC, iOS는 개발 빌드가 서야), 3D 석 장(`no-schedule`·`all-clear`·`server-error`), 로컬 Supabase 구글 프로바이더. `ui-kit`이 남긴 사실 — gorhom 바텀시트는 Jest 대역 위에서 죽어 면(`BottomSheet`)과 제스처 컨테이너(`DraggableSheet`)로 갈렸다. SVG에 `var(--color-*)` 문자열을 넘기면 Tailwind가 그 변수를 `:root`에 내놓아 다크가 깨진다 — `SvgPaint` + `fill-*`/`stroke-*` 유틸로 간다(components.md에 적을 값). 자동 리뷰 잡이 코멘트 없이 초록이었다([관찰 018](observations/018-review-job-passes-without-comment.md)).
