...
근무 요청 UI 시안 검수를 이어간다. 달력의 색 축은 #269에서 닫혔고(위), 남은 것은 문구·부품 쪽 「제안」 판정이다 — worker에 넷(요청 시트 제목 짜임, CTA 배치, 끝난 요청 문구, 늦은 수락 오류 블록 모양)과 admin에 다섯(요청 대기 배지 문구·모양, 픽커 미신청 줄 상태 문구·체크박스 유무, 체크박스 색, 보내기 체크·버튼 모션, 관리자 푸시 시점). 시안 캡션이 붙인 가정 둘(끝난 시트의 제목 유지, 요청 시트 배경 달력의 시점 — 뒤엣것은 #269에서 「근무 점보다 앞선 시점」으로 그려뒀으니 그 가정을 확정할지만 정하면 된다)도 같이 본다.

그게 끝나면 화면 미정이 바닥난다 — backlog 「다음」의 근무자 대시보드 spec, 또는 로드맵 순서의 다음 화면(⑤가입 대기 목록 등)으로 간다.
...
- 근무자: ③급여 조회(주/월/연, 확정 없이 재계산되는 예상치) ④프로필(이름 입력·수정, 승인 전에도 열림) — ①근무표 보기는 봉인, ②는 별도 화면이 아니게 됐다(①의 확정 전 모드), 근무 요청 수락도 ①의 시트로 들어갔다
- 근무자 파생: 교대 요청 받는 쪽 화면(수락·거절)이 목록에 없다 — 절차가 확정돼 필요해졌다. 순서 정할 때 같이 본다
