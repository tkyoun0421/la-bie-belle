...
**근무자 ①근무표 보기 디자인이 2026-09-07에 봉인됐다.** 태관이 비교 시안(스크래치)을 세 라운드 고치며 구조를 정했다 — 월 달력 + 바텀 시트, 아이콘 알약 보기 전환(달력 순 기본/포지션 순), 포지션 순은 날짜 아코디언(줄 오른쪽에 내 포지션/근무 없음/교육), 「내 배정만」 체크박스는 포지션 순에만, 교육 점 구분 걷음, 내 근무 날 문 둘(근무 취소 secondary·교대 요청 primary). 도메인도 같이 개정됐다 — 근무 취소가 확정 뒤 바꾸는 길 다섯째(사유 필수·관리자 승인), 교대는 여럿에게 걸고 수락 여럿이면 관리자가 골라 승인(swap.md 미정 첫 항목이 닫힘), 관리자에게만 보내는 길 신설, 알림 아홉째(근무 취소 단계). components.md 날짜 칸 표가 면 채움으로 개정되고 관리자 편집 상태 표가 합류했다. 페이지 문서 `pages/schedule-worker.md`가 섰고 sian-writer가 시안을 그리는 중이다 — **시나리오 보기(장면 4 순서로 한 장씩 넘김)가 이 시안부터 들어간다**(태관 요청). 시안 검수가 다음 첫 수다.

관리자 쪽 파장 — 교대 수락자 중 고르기와 근무 취소 승인이 관리자 화면 어딘가에 서야 한다. ⑦사유 승인·거절을 그릴 때 같이 본다. 남은 「제안」: 관리자 푸시 시점(예식 3일 전 저녁 9시).

그다음은 남은 화면의 순서다 — 「그리면서 정한다」로 열어뒀다. ②휴무 희망 제출이 이 화면의 확정 전 모습 미정과 얽혀 자연스러운 다음 후보다.
