...
**`globals.css`가 `tokens.md`에서 생성된다.** `pnpm tokens:css`가 표를 읽어 `src/app/globals.css`를 통째로 쓴다. 8절 「CSS 전문」은 표로 담을 칸이 없는 넷(뼈대, Tailwind 기본값 초기화와 서체, shadcn 다리, `@layer base`의 body)만 남았고 나머지는 표에서 생성된다. `tests/lint/generate-globals-css.test.ts`가 생성 결과와 저장된 파일을 통째로 대조한다.

**7절 대비값도 기계가 잰다.** `tests/lint/contrast-check.ts`가 1절 팔레트 hex에서 WCAG 공식으로 다시 계산해 「측정한 조합」과 「떨어진 조합」을 대조한다. 표 파서(`Row` 타입·`readRows`·`requireRows`·`bySection`·헤더 상수)는 `scripts/tokens-md.mts`로 빠져 생성기와 검사가 같이 쓴다.

생성기와 검사 스크립트는 `.mts` 확장자로 `node --experimental-strip-types`로 돈다.

**근무자 대시보드의 디자인이 정해졌다.** `docs/design-system/pages/dashboard.md`에 대시보드·출근 인증·사유 시트 셋이 담겼다. 대시보드는 카드를 안 쓰고 하루 띠가 화면의 무게를 가져간다. 사유 시트는 열 가지 상태(빈 칸·다섯 자 미만·200자 상한·보내는 중·재진입·키보드·반려 등)를 갖췄다. 관리자 대시보드, 스플래시, 각 카드를 눌렀을 때 가는 화면은 이번에 안 담았다.

**시안 HTML을 저장소에 커밋하는 규칙이 이번에 처음 섰다.** `pages/dashboard.sian.html`이 `pages/dashboard.md` 옆에 있다. 시안은 참고고 정본이 아니다 — 어긋나면 `tokens.md`와 페이지 문서가 이긴다. `pages/login.md`는 이 규칙 이전에 쓰여서 짝이 되는 시안 파일이 없다.

`plan.md`의 「다음」에 코드 task 둘이 있다. 「로그인 화면과 승인 대기 화면을 만든다」는 지금 바로 잡을 수 있고, 「근무자 대시보드를 만든다」는 근무표·출근 인증 기록·급여 계산·알림 데이터가 없어 아직 못 연다.

코드는 세션 기반이 서 있고 화면은 아직 없다. 브라우저용 Supabase 클라이언트도 아직 없다.

## 다음 첫 수

`plan.md`의 「다음」에 남은 첫 task는 「로그인 화면과 승인 대기 화면을 만든다」다. `pages/login.md`가 적은 두 화면을 실제로 세운다 — 로그인하면 승인 상태에 따라 대기 화면이나 홈으로 갈린다.

이 task의 완료 조건이 이번에 세운 두 규칙을 처음 실전에 놓는다. 하나는 검수 루프다 — `design-system/README.md`가 「지적마다 이 디렉터리의 조항을 인용해야 반려가 성립한다」고 정했는데 아직 한 번도 안 돌았다. 반려가 실제로 성립했는지, 인용할 조항이 없어서 못 반려한 자리가 있었는지를 로그에 남긴다. 다른 하나는 시안 커밋 규칙이다 — `pages/login.md`엔 짝이 되는 시안 파일이 없으니(규칙이 대시보드부터 적용됐다) 이번엔 문서만으로 구현이 됐는지가 그대로 드러난다.

「근무자 대시보드를 만든다」는 이번 첫 수가 아니다. 대시보드가 그리는 넷(안 본 알림, 출근 인증, 이번 주 근무, 내 급여)이 전부 서버 데이터고, 그 데이터를 만드는 task가 아직 `plan.md`에 없다.

## 열린 결정

- **대시보드 시안의 알림 목업에 「박서연」이라는 이름이 하드코딩돼 있다.** `dashboard.md`와 `dashboard.sian.html` 양쪽에 있다. 실명인지는 확인되지 않았다.
...
- **되돌리기 어려운 동작에 별도 색을 줄지.** 출근 인증과 교대 수락 둘 다 되돌릴 길이 없는데 지금은 같은 `bg.brand-solid`라 한 화면에 브랜드 버튼이 둘 뜰 수 있다. 대시보드의 인증 완료 체크도 같은 브랜드 색을 골랐다. `tokens.md`의 「아직 안 정한 것」에 있다.
- **브랜드 색 출처.** brand 계열이 공식 브랜드 가이드가 아니라 2026 리뉴얼 홀 이미지와 웹사이트 내비게이션에서 뽑은 값이다. 로고나 명함, 인쇄물에 정해진 색이 있으면 hue를 그쪽으로 옮기고 brand 계열을 다시 뽑는다. `tokens.md`의 「아직 안 정한 것」에 있다.
...
- `docs/design-system/tokens.md`의 "세그먼트 목록" — 실제 파일을 보고 정한다.
...
- **`.prettierignore`가 `*.md`와 `*.sian.html`을 거른다.** 문서에 prettier를 돌려도 아무 일도 안 한다. 시안 HTML은 사람이 고른 모습 그대로 남아야 해서 같이 뺐다.
...
- 시안 HTML(`pages/*.sian.html`)은 참고고 정본이 아니다. 값은 `tokens.md`에, 짜임과 문안은 페이지 문서에 있다. 어긋나면 그 둘이 이긴다. 구현할 때 시안의 CSS를 앱으로 옮기지 않는다 — shadcn/ui와 Tailwind 위에 토큰을 갈아끼운다.
