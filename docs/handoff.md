...
- **목록에도 그림이 붙는다** — 근태 인증률 카드에 [도넛](2-design/design-system/components.md#도넛), 그 아래에 [비율 띠](2-design/design-system/components.md#비율-띠). 사람별 줄의 막대는 「몫 35%」 숫자가 받았다
...
- **그림이 브랜드를 쓰는 조건은 화면에 있다**([foundation/color.md](2-design/design-system/foundation/color.md#차트가-색을-쓰는-법)). 눌리는 브랜드가 없는 화면이면 그림이 브랜드고 있으면 차트 계열이다 — 통계가 앞, 대시보드가 뒤다. 조각 넷의 정본은 [components.md의 차트 넷](2-design/design-system/components.md#차트-넷)이고 **등장 모션이 없다**
...
- **화면 문서를 안 갈랐다.** 두 화면이 세그먼트·추이 그래프·요약 판·비율 띠를 전부 공유하고 갈리는 것은 담는 값과 탭 이름뿐이다 — 파일을 가르면 같은 짜임이 두 벌 선다
...
- **근무 0건인 달은 합계 카드와 날마다 카드가 통째로 사라진다.** `–` 하나만 남는다
...
- **공용 조각 다섯이 `shared/ui`다.** 대시보드와 근무자 통계가 가져다 쓴다

**통계 화면에 담는 것을 늘렸다.** 「빈 공간이 너무 많다」는 지적에서 시작했는데 원인이 여백이 아니라 화면이 답하는 질문 수였다. 합계 하나만 크게 세우고 목록으로 떨어지던 짜임을 카드 넷으로 다시 짰다. 비교 시안을 아티팩트로 보여 승인받은 뒤 정본을 고쳤다. 결정 여섯이다.

- **구획을 카드로 가른다** — 가는 선 머리글이 [Card](2-design/design-system/components.md#card)로 바뀌었다. 근무 탭이 합계·날마다·사람별·포지션 넷이다
- **합계 옆을 비우지 않는다** — [요약 판](2-design/design-system/components.md#요약-판) 2×2(사람·날·1인·하루)와 [사람 겹침](2-design/design-system/components.md#사람-겹침)이 붙는다. 넷 다 같은 두 키에서 파생돼 질의가 안 는다
- **포지션이 줄에서 랭킹 블록이 됐다** — 그 포지션을 채운 사람이 1·2·3위 가로로 서고 4위부터 한 줄씩이다. **건수를 안 센다**(자리가 날마다 같은 꼴로 생겨 뜻이 없다). 세는 것은 날수와 시간이고 동점은 이름 가나다순이다
- **정렬 토글을 열었다** — 사람별과 포지션 둘 다 「많은 순 ↔ 적은 순」이다. 포지션을 뒤집으면 「아무도 안 들어간 자리」가 맨 위로 온다. 사람별도 여는 것은 근태의 지각 순 금지와 같은 축인데 **세는 것이 잘잘못이 아니라 시간**이라 갈랐다
- **근태 탭에 큰 숫자가 생겼다** — [도넛](2-design/design-system/components.md#도넛)이 출근 인증률을 찍는다. 그 값의 정의가 여기서 굳었다 — **분자가 출근과 지각의 합**이고 출근 인정은 안 든다([metrics.md](1-plan/metrics.md)). 사람별 줄에도 인증률 열이 붙어 근무 일수가 다른 사람을 견준다
- **줄 막대가 사라지고 「몫 35%」가 그 자리를 받았다** — 막대는 1등과의 거리를 말하고 몫은 홀 전체에서의 크기를 말하는데, 그 카드가 답할 것은 뒤쪽이다

**기각해둔 조항 둘을 다시 열었다.** 「지난달 대비 증감률을 안 적는다」는 바로 위에 열두 달 그래프가 서 있는 것을 조건으로 열었고, 「차트는 브랜드 색을 안 쓴다」는 **화면 단위 조건**으로 다시 적었다 — 눌리는 브랜드가 같이 서지 않는 화면에서는 그림이 브랜드를 쓴다. 통계가 앞이고 대시보드는 「출근하기」가 있어 뒤다.

따라 움직인 정본이 일곱이다 — `stats.md`(전면 개정), `components.md`(줄 막대 자리에 도넛, 요약 판·사람 겹침 신설, 미니 달력 농도와 폭), `foundation/color.md`, `tokens.md`, `dashboard.md`, `schedule-admin.md`, plan 둘. `stats-admin`의 AC가 아홉에서 열이 됐고 조각이 셋에서 다섯이 됐다.
