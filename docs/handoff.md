...
**대시보드 화면 미정 아홉을 닫는다.** 열일곱에서 여기까지 줄었다 — 「규칙과 부딪힌 자리」 열넷은 앞선 회차에 전부 닫혔고(#279·#281·#282·#283), 지도 SDK(네이버)·홀 좌표(DB 설정 행)·사유 넷·권한 거부까지 이번 회차에 닫혔다.

남은 아홉은 무게가 갈린다. **띠와 카운트다운 쪽 넷이 제일 무겁다** — 카운트다운을 몇 시간 전부터 세나(지금은 자정에 열면 「9시간 뒤」가 뜬다), 4시간 근무처럼 막대가 짧을 때 시각 둘을 어디 두나, 겸임·교육 배정이면 막대가 둘인가 하나인가, 인증 창이 근무보다 한 시간 먼저 닫히는 것을 어디서 알리나. 스크래치 비교 시안이 붙을 자리다.

**알림 쪽 셋은 도메인에 걸친다** — 답한 알림이 언제 목록에서 사라지나, 알림 본문을 누르면 무엇이 되나(CTA와 같게 할지 아무것도 안 되게 할지), 교대를 잘못 수락하면 어디로 가나. 마지막 것은 `swap.md`에 수락을 무를 수 있는지가 없어서 도메인부터다.

**나머지 둘은 인증 화면의 시간 축** — 타일이 오기 전 몇 백 밀리초를 무엇으로 채울지, 인증이 끝나고 저절로 돌아갈지다.

미정이 비면 `dashboard.sian.html`을 문서에 맞춘다 — 시안은 이미 있지만 검수 전 상태라 이 회차의 결정들(인증 화면의 넷째 모습, 하루 띠 세 겹, 44px 닿는 면)이 아직 안 들어갔다.

검수 방식은 근무표와 같다 — 후보를 스크래치 비교 시안으로 나란히 그려 눈으로 고르고 결정만 정본에 옮긴다. 스크래치는 저장소에 안 넣는다.

관리자 쪽 파장 — 교대 수락자 중 고르기와 근무 취소 승인이 관리자 화면 어딘가에 서야 한다. ⑦사유 승인·거절을 그릴 때 같이 본다. 교대 요청 받는 쪽 화면(수락·거절)도 화면 목록에 새로 필요하다.
