...
**대시보드 화면 미정 열일곱을 인터뷰로 닫는다.** 「규칙과 부딪힌 자리」 열넷은 이 회차에 전부 닫혔다(#279·#281·#282·#283) — 여덟은 정본에 조항이나 토큰으로 올라갔고, 셋은 이 화면이 예외로 남되 정본이 그 예외를 알게 됐고, 셋은 애초에 부딪힌 것이 아니었다.

남은 열일곱은 무게가 갈린다. **인증 화면 쪽이 제일 무겁다** — 어느 지도를 쓸지(카카오맵·네이버지도), 홀 좌표를 저장소 밖 어디에 둘지(PUBLIC이라 코드에 못 박는다), 타일이 오기 전 몇 백 밀리초를 무엇으로 채울지, 위치 권한을 거부했을 때(지도 자체가 안 뜨고 브라우저는 다시 안 묻는다), 인증이 끝나고 저절로 돌아갈지. 지도 SDK는 무료 한도와 다크 타일을 우리 팔레트에 맞출 수 있는지가 갈림길이라 `web-researcher`가 먼저 붙는다.

**사유 쪽 넷은 domain이 먼저다** — 보낸 사유를 고칠 수 있나, 언제까지 넣을 수 있나(인증 창은 18:00에 닫히는데 사유 창은 언제인지 없다), 반려된 뒤 몇 번 다시 넣나. `attendance.md`의 「사유 승인이 거절되면」이 안 정해져 화면 문서의 반려 블록이 그 전의 제안으로 서 있다. 화면을 정하려면 도메인부터 정해야 하는 자리다.

**띠와 카운트다운 쪽 넷은 화면 판단이다** — 카운트다운을 몇 시간 전부터 세나(지금은 자정에 열면 「9시간 뒤」가 뜬다), 4시간 근무처럼 막대가 짧을 때 시각 둘을 어디 두나, 겸임·교육 배정이면 막대가 둘인가 하나인가, 인증 창이 근무보다 한 시간 먼저 닫히는 것을 어디서 알리나. 스크래치 비교 시안이 붙을 자리다.

**나머지 넷은 알림과 진입** — 답한 알림이 언제 목록에서 사라지나, 교대를 잘못 수락하면 어디로 가나(`swap.md`에 수락을 무를 수 있는지가 없다), 그리고 앞의 것들이 정해지고 나서 볼 것들이다.
