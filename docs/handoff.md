...
근무 요청 UI 시안 검수가 다음 첫 수다. 시안 갱신(#266)과 근무 신청 「제안」 일곱 승인, 근무 요청 화면 인터뷰(픽커 미신청 줄이 보내는 문·자리 카드 배지+픽커 상태·수락은 근무표 그 날+요청 시트, 알림은 전부 소진 시 관리자 한 번·수락자 변경 알림 제외)까지 끝나 문서와 시안에 들어갔다. 새로 생긴 「제안」 — 요청 시트 제목·CTA 배치, 끝난 요청 문구, 배지 문구·모양, 픽커 상태 문구, 체크박스 색, 보내기 모션 — 을 시안을 보며 판정한다.
