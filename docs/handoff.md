...
**작업 트리가 둘로 나뉘었다.** 저장소 본체에서는 디자인과 문서만 하고 코드는 별도 작업 트리에서 간다. 본체 트리는 `src/`와 `tests/`에 손대지 않고, 개발 트리는 디자인 결정을 내리지 않는다. 트리마다 세션이 하나씩 붙고 둘 다 총괄이다. 규칙은 `CLAUDE.md` 맨 위에 있다. **개발 트리는 아직 안 만들었다** — 경로를 사람이 정한다. `src/`는 그대로 있다.
...
`plan.md`의 「다음」에 개발 트리 몫 task 둘이 있고 순서가 있다. **`globals.css`를 `tokens.md`에서 생성하는 것이 먼저다.** 그게 되기 전까지 본체 트리는 토큰 값을 못 바꾼다 — 8절을 고치면 `globals.css`도 따라 움직여야 파리티 테스트가 통과하는데, 본체 트리는 `src/`에 손대지 않기로 했기 때문이다. 7절 대비값 검사는 그다음이다.

개발 트리는 사람이 이미 돌리고 있다. 이 저장소의 `git worktree list`에는 안 잡히니 별도 clone이거나 다른 자리다.
...
- **본체 트리는 `src/`와 `tests/`에 손대지 않는다.** 코드 쪽에서 고칠 것을 발견하면 직접 고치지 말고 문서에 적어 개발 트리로 넘긴다. 반대로 개발 트리는 `docs/design-system/`을 따르고 디자인 결정을 내리지 않는다.
- **`tokens.md` 8절과 `src/app/globals.css`가 파리티 테스트로 묶여 있다.** 8절을 고치면 `globals.css`도 같이 움직여야 `pnpm test`가 통과한다. 이번 회차의 토큰 여섯이 그 경우였다. 본체 트리가 `src/`에 손대지 않기로 한 것과 부딪혀서, 생성기 task가 설 때까지 **본체 트리는 토큰 값을 바꾸지 않는다.** 예외를 뚫는 대신 원인을 없애는 쪽을 골랐다.
