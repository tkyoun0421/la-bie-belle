...
- [ ] 근무자 대시보드 디자인을 정한다 — 근무자가 앱을 여는 이유가 여기 다 있다
  ...
  - 완료 조건: 넷의 자리와 무게가 정해진다. `prd.md`가 "위에서부터 안 본 알림, 출근 인증, 이번 주 근무, 내 급여"라고 순서를 적었는데 그것이 세로 배치인지 시선의 무게인지는 안 정해졌다. 출근 인증 버튼은 근무 시작 한 시간 전부터만 열리니([attendance.md](2-design/domain/attendance.md)) 나머지 시간에 그 자리가 무엇이 되는지도 여기 든다.
  - 완료 조건: 빈 상태 셋이 정해진다. 안 본 알림 0건, 이번 주 근무 없음, 급여 0원이다. `tokens.md`의 「아직 안 정한 것」이 빈 상태 화면을 미정으로 올려뒀고 이 화면이 그 셋을 처음 만난다.
  - 완료 조건: 고른 것이 `docs/2-design/design-system/pages/dashboard.md`에 기록된다. `pages/login.md`가 세운 틀을 따르고, 틀이 안 맞는 자리가 나오면 그 사실을 적는다 — 페이지 문서 둘째 파일이라 틀이 실제로 재사용되는지가 여기서 처음 드러난다.
  - 범위 밖: 관리자 대시보드와 스플래시. 각 카드를 눌렀을 때 가는 화면도 여기 없다.

- [ ] 로그인 화면과 승인 대기 화면을 만든다 — 디자인 문서가 처음 코드가 되는 자리다
  - 완료 조건: [pages/login.md](2-design/design-system/pages/login.md)가 적은 두 화면이 실제로 돈다. 로그인하면 승인 상태에 따라 대기 화면이나 홈으로 갈린다. 세션 기반은 이미 깔려 있으니([spec/session-foundation.md](2-design/spec/session-foundation.md)) 그 위에 화면만 얹는다.
  - 완료 조건: 퍼블리싱이 shadcn/ui와 Tailwind 위에 토큰을 갈아끼우는 방식으로 된다. [design-system/README.md](2-design/design-system/README.md#퍼블리싱)가 정한 대로다. 시안 HTML의 CSS를 옮겨 붙이지 않는다 — 그 파일은 눈으로 보는 것이지 복사할 코드가 아니다.
  - 완료 조건: 검수 루프가 실제로 한 번 돈다. `README.md`가 「지적마다 이 디렉터리의 조항을 인용해야 반려가 성립한다」고 적어뒀는데 아직 한 번도 안 돌았다. 반려가 성립했는지, 인용할 조항이 없어서 못 반려한 자리가 있었는지를 로그에 남긴다 — 디자인 문서가 구현을 실제로 판정할 만큼 촘촘한지가 여기서 처음 드러난다.
  - 완료 조건: 시안 HTML이 구현에 쓰였는지가 로그에 남는다. 문서만으로 충분했는지, 시안을 열어야 알 수 있던 자리가 있었는지다. 이번에 「시안을 문서 옆에 커밋한다」를 규칙으로 세웠으니 그 규칙이 값을 하는지 첫 회차에 확인한다.
  - 범위 밖: 대시보드. 이름 입력 폼과 관리자 승인 화면. 디자인 문서에 「안 담은 것」으로 적힌 자리는 전부 여기도 밖이다.

- [ ] 근무자 대시보드를 만든다 — 디자인은 끝났고 데이터가 없다
  - 완료 조건: 이 task는 앞선 것들이 서기 전에는 못 연다. 대시보드가 그리는 넷(안 본 알림, 출근 인증, 이번 주 근무, 내 급여)이 전부 서버 데이터고, 그 넷을 만드는 task가 아직 `plan.md`에 없다. 무엇이 먼저 필요한지를 이름으로 적어 이 자리에 남긴다 — 근무표, 출근 인증 기록, 급여 계산, 알림이다.
  - 왜: 디자인 문서와 시안이 [pages/dashboard.md](2-design/design-system/pages/dashboard.md)에 다 있는데 붙일 데이터가 없다. 순서를 안 적어두면 화면부터 만들고 가짜 데이터를 박는 길로 간다.
  - 범위 밖: 관리자 대시보드.
...
- [x] 로그인 화면과 승인 대기 화면의 디자인을 정한다 — 눈으로 보고 고른다 — [docs/log/2026-08-29.md](log/2026-08-29.md)
  ...
  - 완료 조건: 승인 대기 화면에 무엇을 담고 무엇을 안 담는지가 정해진다. 로그인한 계정을 드러낼지, 승인까지 얼마나 걸리는지 약속할지, 관리자에게 연락할 길을 줄지가 그 목록이다. 약속할 근거가 없는 문장은 안 쓴다 — 근무표 확정 마감일이 `docs/2-design/domain/schedule.md`에 없어서 "8월 28일에 나옵니다"를 못 쓴 것과 같은 이유다.
  - 완료 조건: 고른 것이 `docs/2-design/design-system/` 아래 기록된다. 페이지별 디자인 문서의 첫 파일이라 그 틀도 같이 정해진다 — `README.md`가 산출물로 적어뒀는데 실제 파일이 아직 하나도 없다.
...
- [x] 세션 기반을 깐다 — 계정 영역의 나머지가 전부 이 위에 선다 — [docs/log/2026-08-28-2.md](log/2026-08-28-2.md)
  - 완료 조건: [docs/2-design/spec/session-foundation.md](2-design/spec/session-foundation.md). 로그인한 사람을 서버와 브라우저 양쪽에서 읽고 토큰을 만료 전에 갱신한다. 화면은 안 만든다.
...
- [x] 디자인 레퍼런스 검토 — 시안 방향을 사람이 정한다 — [docs/log/2026-08-26-4.md](log/2026-08-26-4.md)
  - 완료 조건: 방향 결정과 근거 레퍼런스가 `docs/2-design/design-system/`에 기록된다.
- [x] 교대와 출근과 알림 규칙을 확정한다 — [docs/log/2026-08-26-3.md](log/2026-08-26-3.md)
  - 완료 조건: `docs/2-design/domain/swap.md`와 `docs/2-design/domain/attendance.md`와 `docs/2-design/domain/notification.md`의 "아직 안 정한 것"이 빈다. QR 위조 대응과 iOS PWA 푸시 제약 대응이 여기 들어간다.
- [x] 계정과 근무표 규칙을 확정한다 — [docs/log/2026-08-26-3.md](log/2026-08-26-3.md)
  - 완료 조건: `docs/2-design/domain/account.md`와 `docs/2-design/domain/schedule.md`의 "아직 안 정한 것"이 빈다. 첫 관리자 부트스트랩, 가입 거절, 퇴사자 보존, 근무자가 남의 근무를 보는 범위 넷이 여기 들어간다.
- [x] 급여 규칙을 확정한다 — 법정 기준 확인을 포함한다 — [docs/log/2026-08-26-3.md](log/2026-08-26-3.md)
  - 완료 조건: `docs/2-design/domain/payroll.md`의 "아직 안 정한 것"이 빈다. 연장 기준 시간, 가산 중복, 야간 가산, 주휴수당, 휴게 공제, 휴일 8시간 초과분 여섯을 근로기준법과 맞춰 정하고 어긋난 자리는 왜 그렇게 정했는지를 같이 남긴다. 세전·실지급 구분과 주 경계·지급일·시급 소급도 정해진다.
- [x] 도메인 규칙의 집을 `docs/2-design/domain/`으로 정한다 (ADR-004) — PRD에서 규칙을 빼고 영역별 파일 여섯으로 나눴다 — [docs/log/2026-08-26-2.md](log/2026-08-26-2.md)
