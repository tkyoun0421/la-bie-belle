...
어느 작업 트리 몫인지는 제목 끝에 적는다. 안 적으면 본체 트리다. 트리를 왜 나눴는지는 `CLAUDE.md`에 있다.
...
- [ ] `src/app/globals.css`를 `tokens.md`에서 생성한다 — 개발 트리
  ...
  - 왜: 지금은 8절과 `globals.css`를 사람이 맞춰야 해서, 디자인 트리가 토큰을 하나 고칠 때마다 `src/`에 손을 대야 한다. 트리를 나눈 규칙과 정면으로 부딪히는 자리다. **이게 되기 전까지 본체 트리는 토큰 값을 못 바꾼다.**
  - 범위 밖: 토큰 값 자체를 바꾸는 것. 7절 대비값 검사는 아래 별도 task다.

- [ ] `tokens.md` 7절의 대비값을 기계가 재게 한다 — 개발 트리
