...
- [ ] 로그인 화면과 승인 대기 화면을 만든다 — 디자인 문서가 처음 코드가 되는 자리다
  - 완료 조건: [pages/login.md](design-system/pages/login.md)가 적은 두 화면이 실제로 돈다. 로그인하면 승인 상태에 따라 대기 화면이나 홈으로 갈린다. 세션 기반은 이미 깔려 있으니([spec/session-foundation.md](spec/session-foundation.md)) 그 위에 화면만 얹는다.
...
- [x] 근무자 대시보드 디자인을 정한다 — 근무자가 앱을 여는 이유가 여기 다 있다 — [docs/log/2026-08-30.md](log/2026-08-30.md)
