...
- [ ] 로그인 화면과 승인 대기 화면을 만든다 — 디자인 문서가 처음 코드가 되는 자리다
  - 완료 조건: [pages/login.md](design-system/pages/login.md)가 적은 두 화면이 실제로 돈다. 로그인하면 승인 상태에 따라 대기 화면이나 홈으로 갈린다. 세션 기반은 이미 깔려 있으니([spec/session-foundation.md](2-design/spec/session-foundation.md)) 그 위에 화면만 얹는다.
  - 완료 조건: 퍼블리싱이 shadcn/ui와 Tailwind 위에 토큰을 갈아끼우는 방식으로 된다. [design-system/README.md](design-system/README.md#퍼블리싱)가 정한 대로다. 시안 HTML의 CSS를 옮겨 붙이지 않는다 — 그 파일은 눈으로 보는 것이지 복사할 코드가 아니다.
...
- [ ] 근무자 대시보드를 만든다 — 디자인은 끝났고 데이터가 없다
  ...
  - 왜: 디자인 문서와 시안이 [pages/dashboard.md](design-system/pages/dashboard.md)에 다 있는데 붙일 데이터가 없다. 순서를 안 적어두면 화면부터 만들고 가짜 데이터를 박는 길로 간다.
  - 범위 밖: 관리자 대시보드.
