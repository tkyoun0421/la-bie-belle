...
이 문서는 라비에벨의 공통 개발 규칙과 안정적인 `DEV-*` ID의 정본이다. 설계·승인·실행 경계는 [운영 계약](WORKFLOW.md)과 [ADR-0011](adr/0011-planning-radio-development-contract.md), 제품 의미는 [PRD](PRD.md), 도메인 언어는 [Domain](DOMAIN.md), 시스템 구조는 [Architecture](ARCHITECTURE.md), task별 기술 설계는 [`docs/development`](development/README.md), 실행 상태는 [작업 인덱스](phases/index.jsonl)가 소유한다.

## 규칙 등급과 예외

- `MUST`: 위반할 수 없는 계약이다. 가능한 항목은 자동 검증한다. task RADIO에서 면제할 수 없고, 부적절하면 개발 인터뷰에서 규칙과 관련 ADR을 먼저 변경한다.
...
- 제품·프로젝트·도메인·UX와 제품 인수 조건은 기획 인터뷰에서 승인한다.
- `design_pending` task의 구현 구조와 기술 인수 조건은 RADIO 개발 인터뷰에서 승인한다.
- 승인된 RADIO 정본은 `docs/development/<task-id>-radio.md`가 소유한다.
- 개발은 두 승인과 `radio_ref`가 있는 `planned` task 하나를 사용자가 ID로 명시했을 때만 시작한다.
- `.agents/runs/<task-id>/radio.md`는 설계 정본이 아니라 승인된 설계의 적용 결과와 실행 중 차이를 기록한다.
...
- 제품 결정이 새로 필요하면 기획 인터뷰, 기술 결정이 새로 필요하면 개발 인터뷰로 반환한다.
...
- `DEV-TEST-01` `MUST`: 인수 조건마다 다섯 위험 렌즈와 검증 계층을 정한다.
...
모든 task는 위험에 따라 간결·일반·심화 RADIO를 갖는다. 정본 형식은 [task별 개발 설계](development/README.md)를 따른다.
...
- `.codex/hooks.json`의 `SessionStart`는 현재 task와 TDD·검증 계약을 Codex에 전달한다.
- `PreToolUse`는 Codex의 `git commit` 전에 기존 task·TDD pre-commit guard를 실행한다.
...
- Codex hook은 신뢰된 프로젝트에서만 로드한다.
- Git hook은 Codex 밖 commit의 동일한 최종 방어선이며 원격 우회 방지는 CI가 담당한다.
