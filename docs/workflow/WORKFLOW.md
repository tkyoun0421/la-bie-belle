...
이 프로젝트의 모든 작업은 하나의 순차 파이프라인 5단계를 지난다. 사용자는 앞 두 단계에서 무엇을 왜 만들지, 어떻게 만들지를 각각 승인한다. AI는 두 승인을 받은 task 하나만 뒤의 세 단계에서 구현·검증·정리한다.
...
| 5. 리팩토링 | AI 실행 | 동작 변경 없는 구조 정리와 재검증 | 없음 → `done` | 완료 보고 시 |

- 승인 게이트는 1단계와 2단계 종료 지점 두 곳뿐이다. AI는 승인 없이 다음 단계로 넘어가지 않는다.
- 3~5단계는 하나의 `in_progress` 구간이며 사용자가 task ID를 명시했을 때만 시작한다. 자동화는 이 구간을 깊게 담당하고 다음 task를 자동 선택하지 않는다.
...
사용자가 승인된 task ID의 실행을 요청하면 다음을 수행한다.

1. 명시된 task, 두 승인, RADIO 해시와 의존성을 검증한다.
...
5. 결과와 handoff를 사용자에게 보고하고 멈춘다. 다음 task를 자동 선택하지 않는다.
...
- `in_progress`: 사용자가 명시한 task ID 하나를 개발·검증·리팩토링 중.
