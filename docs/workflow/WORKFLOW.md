...
설계 중에 승인된 정본(PRD·DOMAIN·DESIGN·design/**·ADR)과 부딪히는 결정이 나오면 **그 자리에서 문서를 고치지 않는다.** 정본은 다른 승인의 결과라 조용히 고치면 승인 이력이 끊긴다.

대신 그 task의 설계 산출물에 **문서 개정 목록**을 표로 쌓는다. 한 행에 네 칸이다 — `문서:줄` · `지금 적힌 것` · `새 틀` · `근거`(어느 결정·라운드가 낳았는지).

- **행은 부딪힌 그 자리에서 넣는다.** 나중에 몰아서 적으면 근거가 사라진다.
- 나중 결정이 **같은 문서 같은 줄**을 다시 건드리면 행을 늘리지 않고 **덮어쓴다.** 근거는 결정 기록이 갖는다.
- 표는 RADIO의 Architecture로 들어가 봉인된다. **실제 문서 개정은 개발 단계가 한다.**
- 정본이 아니라 **기획이 정해야 할 물음**이면 개정 목록이 아니라 미결로 올려 1단계로 반환한다.

**부딪히면 디자인이 이긴다**(2026-08-18 사용자 결정). 1단계에서 정한 기획이 2단계 설계 중에
흔들리면, 디자인을 기획에 맞춰 접지 않고 **기획 문서를 개정 목록에 올린다.** 화면을 그려보기
전에는 안 보이던 것이 그리는 중에 보이고, 그때 나온 판단이 더 늦고 더 정확하다.

**다만 디자인이 못 정하는 것이 있다.** 아래 넷은 디자인 라운드에서 뒤집지 않고 **1단계로
반환한다** — 그리는 것으로 답이 나오는 물음이 아니라 화면 밖에서 정해져야 하는 것들이다.

- **없는 데이터** — 화면이 받지 않는 값을 전제하는 결정(예: 배정 정보가 안 오는 화면에서
  「내가 배정됐는지」를 그리는 것)
- **권한과 PII** — 누가 무엇을 보고 바꿀 수 있나
- **돈과 시각** — 급여 계산, 근태 시각, 마감 시각처럼 값이 규칙으로 정해지는 것
- **없던 기능** — 기획이 「만들지 않는다」고 적어둔 것을 만드는 결정

이 넷은 개정 목록에도 올리되 **「기획 승인 먼저」**를 함께 적는다.

**자동 검사는 없다.** `gate:docs`는 의존 순환·제목 일치·spec 참조·내부 링크 넷만 본다(`harness/lib/docs-check.ts`). 이 표가 유일한 장부다.
...
0. [`publisher`](../../.claude/agents/publisher.md)를 먼저 **읽기 전용으로 돌려 대조만 시킨다.** 시안·NOTES가 그리라는 값과 봉인된 모델이 주는 값을 맞대고, **화면이 못 만드는 값을 하나도 빠짐없이 한 번에** `[질문]`으로 낸다. 파일은 하나도 만들지 않는다.
...
**0번이 있는 이유.** 반환은 정상 경로지만 **만들다 막혀서 돌아오는 것**은 비싸다. 비싼 쪽은 왕복 자체가 아니라 **재독**이다 — 돌아올 때마다 RADIO·NOTES·시안을 처음부터 다시 읽는다. P0-T48 5B(홈)에서 이 순서를 안 지켜 반환이 세 번 났고, 약 64분을 쓰고도 화면 코드가 0줄이었다(`publisher`가 18분 쓰고 파일 0개). 두 번째 반환에 「하나씩 말고 한꺼번에 내라」를 붙이자 남은 넷이 한 번에 나왔다. 0번은 그 지시를 처음부터 주는 자리다.

**이미 돌던 worker는 새로 띄우지 말고 이어서 부른다.** 같은 `publisher`를 이어 부르면 컨텍스트가 살아 있어 재독이 없다 — 같은 라운드에서 처음 호출 611초, 이어 부른 호출 45초였다.
