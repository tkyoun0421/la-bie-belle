...
- 공통 개발 원칙은 `docs/DEVELOPMENT.md`의 안정적인 `DEV-*` ID가 소유한다. 규칙은 `MUST`, `SHOULD`, `MAY`로 구분한다.
- RADIO는 공통 규칙을 반복하지 않고 `기본 적용`, `해당 없음`, `추가 결정`, `예외` 상태와 작업별 차이만 기록한다.
- 승인된 RADIO 정본은 `docs/development/<task-id>-radio.md`, 실행 결과는 `.agents/runs/<task-id>/radio.md`가 소유한다.
