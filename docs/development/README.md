...
이 디렉터리는 개발 인터뷰에서 승인한 task별 기술 설계의 정본이다. 공통 규칙은 [개발 컨벤션](../DEVELOPMENT.md)을 참조하고 반복하지 않는다.

## 파일과 상태

- 파일명은 `<task-id>-radio.md`를 사용한다.
- 문서에 1부터 시작하는 `revision`과 `Draft | Approved` 상태를 기록한다.
...
- 승인 후 의미가 바뀌면 고정 경로에서 revision을 증가시키고 `Draft`로 바꾼 뒤 개발 인터뷰에서 재승인한다. 이전 revision은 Git 이력으로 보존한다.
- 여러 task에 영향을 주거나 되돌리기 어려운 결정은 ADR로 승격한다.
- `.agents/runs/<task-id>/radio.md`는 이 문서의 대체물이 아니라 실행 결과와 차이 증거다.
