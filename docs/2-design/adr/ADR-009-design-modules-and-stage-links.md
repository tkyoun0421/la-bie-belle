...
관심사가 폴더고 도메인이 파일인 배치는 한 회차가 한 폴더만 열게 하려는 것이었다(ADR-004, [system/architecture.md](../system/architecture.md)). 실제로는 한 영역의 계약이 다섯 곳으로 갈라졌고, 같은 행위를 두 문서가 다르게 적어도 아무도 못 잡았다 — 연락처 수정의 저장 경로가 api와 runtime에서 달랐고, 「통신 지연」의 비교 기준이 domain과 runtime에서 달랐다. 한 회차가 여는 것은 관심사가 아니라 업무였다.
