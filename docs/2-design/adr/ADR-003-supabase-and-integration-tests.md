...
가입 승인도 여기서 막는다. 승인되지 않은 계정은 정책에 걸려 행을 못 받는다. 화면에서 숨기는 것과 다르다.

**자기 프로필은 예외다.** 승인 여부와 상관없이 본인은 자기 프로필을 읽고 고친다. 가입한 사람이 이름을 넣는 화면이 자기 행을 읽어야 하는데, 승인을 그 앞에 두면 이름 없는 프로필만 쌓이고 관리자는 누구를 승인할지 모른다. 승인은 자기 프로필 바깥의 모든 것을 막는다 — 근무표, 급여, 남의 프로필이 여기 속한다.
