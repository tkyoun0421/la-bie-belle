...
- 기본 계약:
  ...
  - **완전히 새로 뜨는데 오프라인이면 「통신 없음」 화면이다.** 껍데기는 Service Worker가 주니 흰 화면은 아니다
  - 띠는 `navigator.onLine`이 아니라 **실제 요청 실패**로 뜬다. 다시 읽기가 성공하면 띠가 진다
  - **쓰기는 큐에 넣지 않는다.**
- 이유: 홀에서 「오늘 누가 오나」를 보는 게 주 용도라 그 화면이 오프라인에서도 보여야 한다. `['profile']`을 영속하지 않아 승인·차단을 가를 수 없다. `onLine`은 와이파이에 붙었지만 인터넷이 안 되는 상태를 못 가른다. 언제 갈지 모르는 쓰기는 사람이 「됐다」고 믿고 가버리는 것이 문제다
- 예외: 출근 인증만 예외적으로 화면이 열린 동안 다시 시도한다 — [`attendance/design.md`](../modules/attendance/design.md)

## 시각

- 적용 범위: 화면이 그리는 모든 시각
- 기본 계약: **앱이 뜨면 `server_now()` 한 번, 그 차이를 기기 시계에 더해 쓴다.** 탭 복귀 때 다시 받는다. 오프셋은 `localStorage`에 남겨 오프라인으로 뜨면 마지막 값을 쓰고, 없으면 0이다. 하루 띠·카운트다운·버튼 켜짐이 이 값이고 매초 도는 것은 로컬 계산이다. 판정은 함수의 `now()`다 — [서버 시각](#서버-시각)
- 예외: 출근 인증의 `reported_at` 하나다

### 서버 시각

- 적용 범위: 앱이 뜬 뒤 화면이 읽는 시각
- 기본 계약: **앱이 뜨면 `server_now()`를 한 번 부르고 차이를 든다.** `offset = server - Date.now()`. 화면은 `Date.now() + offset`을 쓴다. 탭이 돌아올 때(`visibilitychange`) 다시 받는다. 화면 시각은 보여주기용이다. 버튼이 켜지는 것, 카운트다운, 남은 시간이 여기 걸리고 판정은 함수의 `now()`다. 언제 다시 받고 오프라인이면 어쩌나는 [시각](#시각)이 정한다
...
- 질문: iOS 홈 화면 앱에서 `visibilitychange`가 앱 전환마다 오는지
- 영향: 탭 복귀 재조회와 시각 재동기화가 이 이벤트에 산다
- 필요한 근거와 대안: 기기 테스트
