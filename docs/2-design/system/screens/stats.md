...
- 다른 영역 — [payroll.md](../../modules/payroll/screens/payroll.md), [wages.md](../../modules/payroll/screens/wages.md), [members.md](../../modules/account/screens/members.md), [schedule-worker.md](../../modules/schedule/screens/schedule-worker.md), [dashboard.md](dashboard.md), [prd.md](../../../1-plan/prd.md), [metrics.md](../../../1-plan/metrics.md)
...
| --- | --- | --- | --- | --- |
| 인건비 | 세그먼트의 「인건비」 | 앱바, 달 줄, 세그먼트, 합계, 예상치 안내, 보조 줄, 사람별 목록 | 달 줄의 좌우 화살표, 세그먼트 | 근태, 포지션 |
| 근태 | 세그먼트의 「근태」 | 앱바, 달 줄, 세그먼트, 현황 줄, 사람별 목록 | 달 줄의 좌우 화살표, 세그먼트 | 인건비, 포지션 |
| 포지션 | 세그먼트의 「포지션」 | 앱바, 달 줄, 세그먼트, 합계, 포지션 목록 | 달 줄의 좌우 화살표, 세그먼트 | 인건비, 근태 |
| 빈 상태 | 그달에 확정된 근무표가 없다 | 합계 자리는 `–`, 목록 자리에 빈 상태 제목과 아래 줄 | 달 줄의 좌우 화살표, 세그먼트 | 세 탭이 같은 모양으로 빈다 |
...
| 빈 상태 제목 | 이 달은 아직 근무표가 없어요 | 빈 상태 |
| 빈 상태 아래 줄 | 근무표를 확정하면 여기 숫자가 서요 | 빈 상태 |
...
**사람별 날짜 내역.** 위 [사람별 목록](#인건비-사람별-목록)에 근거가 있다.
...
- 질문: 포지션별 시간에 막대를 붙일지
- 필요한 근거와 대안: 지금은 숫자와 정렬만으로 비교하게 둔다. 막대를 넣으면 아홉 줄이 전부 색 면이 되는데 이 앱에 그런 자리가 [하루 띠](../../design-system/components.md#하루-띠) 하나뿐이다

#### Q-02

- 질문: 인건비에 시급 이력이 걸친 달의 표기
- 필요한 근거와 대안: 달 중간에 시급이 바뀌면 한 사람 몫이 두 단가로 계산되는데, 목록 줄이 그 사실을 말할지 안 정했다

#### Q-03

- 질문: 근무자에게 포지션별 시간을 열지
- 필요한 근거와 대안: 지금은 관리자만 본다. 자기가 어느 포지션에 얼마나 들어갔는지는 근무자도 궁금할 수 있다

#### Q-04

- 질문: 빈 상태에서 예상치 안내와 보조 줄을 같이 지울지
- 필요한 근거와 대안: 문서는 「합계 자리는 `–`로 서고 목록 자리에만 빈 상태가 온다」까지만 정했다. 시안은 근무 0건일 때 「근무 0건 · 0시간」이 서는 것이 이상하다고 보고 지우는 쪽으로 그렸다
