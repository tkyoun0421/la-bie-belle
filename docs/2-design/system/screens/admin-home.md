...
들어오는 문은 둘이다. 근무자 탭 넷의 앱바 오른쪽에 관리자에게만 서는 「관리자」 스위치가 주 문이고, [프로필](../../modules/account/screens/profile.md#관리자-모드)의 「관리자 모드」 줄이 또 하나다. 돌아가는 길은 이 화면 앱바의 같은 스위치 하나고 `/`에 선다 — 뒤로는 없다([navigation.md](../../system/navigation.md#세-층)). 관리자에게 승인과 근무표가 자기 근무보다 잦아 진입이 한 번이어야 한다([account/README.md](../../modules/account/README.md#acc-008)).
...
1. 앱바 — 「관리자」, 오른쪽에 [종 아이콘](../../design-system/components.md#종-아이콘)과 「관리자」 스위치. 뒤로는 없다 — 스위치가 `/`로 돌아가는 유일한 길이다
2. 오늘 현황
3. 근무표 관리 타일
4. 빈 자리 카드
5. 이번 달 근무표 미니뷰
