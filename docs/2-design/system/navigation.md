...
- 필요한 근거와 대안: 앱이 깔렸으면 앱이 열고 아니면 웹이 열리는 주소를 쓰면 받을 곳을 안내할 수 있다. 다만 그 안내를 띄울 페이지가 하나 필요해서 [ADR-011](../adr/ADR-011-expo-native-app.md)의 「웹은 같이 안 낸다」와 닿는다 — 화면이 아니라 안내 한 장이라 같은 것인지 총괄이 정한다. 앱만 여는 주소를 쓰면 안 깐 기기에서 아무 일도 안 일어난다. 근무자가 앱을 깔고 시작하는 것이 전제라 실제로 이 자리에 서는 사람이 있는지부터 본다
