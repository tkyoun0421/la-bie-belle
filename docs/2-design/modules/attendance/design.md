...
**앱 안에 스캐너가 없다.** 종이의 QR이 담는 것은 코드 문자열이 아니라 앱 주소다 — `/check-in?c=<코드>`. 근무자는 기기 카메라 앱으로 벽의 종이를 찍고, 열린 링크가 앱을 그 주소로 데려오고, 앱이 들어오자마자 `check_in`을 `method = 'qr'`로 부른다. 카메라 권한을 안 받는 길이고, 위치 권한을 이미 거부한 사람에게 권한 창을 한 번 더 띄우지 않는다. 대신 코드가 주소에 실려 브라우저 기록에 남는다 — [ATT-027](README.md#att-027)이 이미 연 구멍과 같은 결이고, 그 구멍이 주소 복사만큼 넓어진다.

로그인이 안 된 채로 링크를 열면 로그인 뒤 그 주소로 돌아온다. 코드는 주소에 남아 있고 인증은 돌아온 자리에서 일어난다.
...
- 입력·전제: 관리자 화면의 「이미지 내려받기」. 서버에 그림 파일을 두지 않는다 — **브라우저가 그 자리에서 그린다**. `['hall', 'qr']`로 받은 값을 캔버스에 QR로 그리고 제목·안내·자르는 선을 얹어 2480×3508 PNG를 만든다. 파일을 저장해 두면 `rotate_qr` 뒤에 옛 그림이 남고, 그 종이가 붙은 채로 근무자가 `invalid_qr`을 받는다
- 읽고 쓰는 데이터: 읽기만이다. 만든 그림을 어디에도 안 남긴다
- 권한: `hall_secrets`를 읽는 자리라 관리자만이다
- 결과와 실패: 브라우저가 파일 저장을 안 열면 같은 그림을 새 탭에 띄운다([qr.md](screens/qr.md#이미지-내려받기)). 그림을 못 그리는 실패는 따로 다루지 않는다 — 캔버스가 없는 환경이면 화면의 QR도 못 그린다
- 캐시 갱신: 없다
