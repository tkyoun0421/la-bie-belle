...
**링크가 앱을 열려면 도메인이 한 장 필요하다.** 우리 주소를 우리 앱이 받는다는 것을 기기에 증명하는 파일을 그 도메인에 올려둬야 아이폰과 안드로이드가 그 링크를 앱으로 넘긴다. 파일이 없거나 앱이 안 깔린 기기는 그 주소를 브라우저로 연다 — 앱을 안 깐 기기가 벽의 종이를 찍으면 무엇을 보는지는 [system/navigation.md](../../system/navigation.md#q-03)에 열려 있다.
...
- 결과와 실패: 창 밖이면 `window_closed`, 반경 밖이면 `too_far`, 옛 코드면 `invalid_qr`, 그날 배정이 없으면 `not_allowed`, 이미 찍혀 있으면 `already_done`이다. **검사가 거친 것부터라** 둘이 같이 틀리면 앞의 코드가 나간다([함수 안의 규칙](../../system/data-access.md#함수-안의-규칙)) — `too_far`와 `invalid_qr`은 창이 열린 시각에만 나오고 창 밖은 전부 `window_closed` 하나다. 문안이 이 사실에 기댄다. 화면은 보내는 동안 「인증을 보내는 중」을 보인다. 다섯 번 다 실패하면 「인증을 못 보냈어요. 통신이 되는 곳에서 다시 눌러주세요」다
- 캐시 갱신: `['attendance', 그날]`
