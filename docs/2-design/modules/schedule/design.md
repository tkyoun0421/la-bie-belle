...
날 닫기만 닫는 모양이 다르다. 자리가 지워지고 `slot_id`의 `on delete cascade`를 타고 `requests`가 따라 사라진다 — `closed_at`을 찍어봐야 같은 트랜잭션에서 행이 없어진다. 확정 전에만 닫히니 지워도 되는 자리다. 그 요청을 가리키던 알림은 없는 행을 만나고, 받는 쪽이 그것을 견딘다.
