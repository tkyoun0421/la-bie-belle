...
이름 변경(`set_display_name`)과 연락처 변경(`profile_private` 직접 갱신)은 즉시 칠하고 실패면 되돌린다. 승인·거절·차단·역할 변경은 응답을 기다린다 — 남에게 닿는다.
...
목표와 지금 코드가 다른 자리다. 문장은 각 절에 있고 여기는 가리키기만 한다.

- [프로필 신원](#프로필-신원) — `profiles.id`와 `user_id`의 분리. [계정 데이터 구조 전환](../../../backlog.md)
- [개인정보는 표를 가른다](#개인정보는-표를-가른다) — `profile_private` 분리. [계정 데이터 구조 전환](../../../backlog.md)
- [첫 진입과 게이트](#첫-진입과-게이트) — 서버 판정을 클라이언트로. [인증 진입 전환](../../../backlog.md)
- [UI 연결](#ui-연결) — `readAuthGate`·`getApprovedAt` 이동. [인증 진입 전환](../../../backlog.md)
