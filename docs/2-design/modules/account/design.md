...
공개로 두는 것은 사진을 승인된 전원이 목록과 픽커에서 `<img>`로 그리기 때문이다. 서명 URL을 매번 받으면 화면마다 호출이 늘고 오프라인 캐시가 어렵다. 구글 프로필 사진도 공개 URL이라 같은 급이다. URL은 uuid가 들어 있어 추측으로는 못 연다.

**브라우저가 줄여서 올린다.** 512px 정사각 webp로 줄인 뒤 올리고, 버킷의 파일 크기 상한은 1MB다. 폰 원본이 그대로 올라가면 목록에서 사진 하나가 수 MB가 된다. 옛 파일은 지우지 않는다 — 지우는 호출이 실패해도 새 사진은 이미 섰고, 정리는 나중 일이다.
