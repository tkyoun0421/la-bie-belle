...
**본인이 올린 사진은 Supabase Storage의 `avatars` 버킷에 산다.** 공개 읽기 버킷이고, 쓰기(insert·update·delete)는 `<user_id>/` 폴더에 본인만이다. 파일 이름은 `<user_id>/<uuid>.webp`고 `photo_url`에는 그 공개 URL이 들어간다.
...
**앱이 줄여서 올린다.** 고른 사진을 512px 정사각 webp로 줄인 뒤 올리고, 버킷의 파일 크기 상한은 1MB다. 폰 원본이 그대로 올라가면 목록에서 사진 하나가 수 MB가 된다. 옛 파일은 지우지 않는다 — 지우는 호출이 실패해도 새 사진은 이미 섰고, 정리는 나중 일이다.
