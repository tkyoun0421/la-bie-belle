...
용어와 규칙의 정본이다. 문서와 코드와 화면 문구가 같은 말을 쓰고, 계산과 상태 전이는 여기 적힌 것을 따른다. 자리와 근거는 `docs/2-design/adr/ADR-004-domain-rules-home.md`에 있다.

PRD는 왜 만드는지를 담고 규칙은 안 담는다. `docs/2-design/spec/`은 task 하나가 무엇이 되면 완료인지를 담는다. task가 끝나도 남는 문장이면 여기가 집이다.
