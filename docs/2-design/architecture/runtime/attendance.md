...
짝은 [domain/attendance.md](../../domain/attendance.md)다. 키는 `['attendance', 'YYYY-MM-DD']`(그날 `check_ins`와 `excuse_status`)·`['excuses', 'YYYY-MM']`(본인 사유 목록)·`['hall', 'qr']`다. 명단은 이 키와 `['schedule', 그달]`의 배정을 합쳐 그린다. 무효화는 [`README.md`](README.md#무효화-표)에 있다.
...
**판정은 누른 시각이다.** 화면이 `reported_at`(`Date.now() + 오프셋`)을 보내고 함수가 그 값으로 지각을 가른다 — 지하에서 09:58에 눌렀는데 10:04에 닿아도 정시다. 「인자로 시각을 받지 않는다」([`../api/`](../api/#서버-시각))의 유일한 예외라 한도가 붙는다. `reported_at`이 `now()`보다 10분 넘게 이르면 `now()`로 판정하고, `now()`보다 늦으면 `now()`다. 기기에서 보내는 값이라 고쳐 보낼 수 있는데, 한도가 그 폭을 10분으로 막는다. 10분은 재시도 1분과 화면 잠금 뒤 재개를 덮는다. 저장은 둘 다다 — `checked_at`이 판정에 쓴 시각, `reported_at`이 기기가 보낸 원값. 둘이 5분 넘게 다르면 명단이 「통신 지연」을 표시한다.
