...
`proxy`가 세션 쿠키만 본다 — 없으면 `/login`이다. 승인됐는지·차단됐는지·퇴사했는지는 앱이 뜬 뒤 `['profile']`을 읽고 가른다. 이유는 [`README.md`](README.md#캐시-네-계층)에 있다 — 서버가 판정을 그리면 껍데기를 캐시할 수 없다. 지금 코드는 `src/middleware.ts`가 세션을 갱신하고 `src/features/auth/read-auth-gate.ts`가 서버에서 `approved_at`을 읽어 가른다 — `backlog.md` 「인증 진입 전환」이 옮긴다.
