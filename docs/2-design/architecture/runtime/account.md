...
`['profile']`이 `blocked_at`을 들면 차단 화면, `left_at`을 들면 퇴사 화면, `approved_at`이 없으면 `/pending`이다. `staleTime`이 0이고 영속하지 않는다 — 차단당한 사람이 옛 프로필로 근무표를 더 보는 일이 없게. 읽는 동안은 아무것도 안 그린다.

## 로그인 뒤

`ensure_profile()`을 부르고 `['profile']`을 읽는다. 없던 사람이면 `/pending`의 프로필 입력 폼이다. 카카오 로그인은 `auth.signInWithOAuth`가 페이지를 떠났다 돌아오니 돌아온 자리에서 이 순서가 돈다.
