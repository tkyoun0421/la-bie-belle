...
- 전제: 관리자
- 행동: `/admin`을 연다
- 관찰 결과: 근무표 관리 타일에 지금 달 상태가 한 줄로 선다 — 근무표 없음 / 만드는 중 / 확정 뒤. 그 아래 근무 시간 기본값 줄이 이 화면 안에서 시트를 열고, 줄 여섯(승인할 일·가입 대기·직원·시급·QR·통계)이 각자의 경로로 간다. 아직 없는 화면으로 가는 줄도 눌린다
- 검증 층: e2e
- 근거: [admin-home.md](../system/screens/admin-home.md)
...
- 제한: 포지션 줄이 배정 수만 세는 임시 줄이라 [schedule-assign](schedule-assign.md)이 그 안쪽을 채우기 전까지 그 줄은 미완이다.
