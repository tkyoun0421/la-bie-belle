...
**가운데 Dialog의 치수가 정본에 없다.** [components.md](../components.md#dialog와-바텀시트)가 토큰 이름만 말하고 폭도 안쪽 여백도 그림자도 안 적어뒀다. 이 앱에서 가운데 Dialog가 실제로 서는 첫 자리라 시안이 값을 재서 그렸고, 그 값을 정본으로 올릴지는 [tokens.md](../tokens.md)의 「빈자리」에서 본다.
