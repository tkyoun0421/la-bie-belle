...
**세그먼트는 [급여 조회](payroll.md#기간-세그먼트)의 그것과 같은 조각이다.** 거기는 기간 단위를 고르고 여기는 볼 것을 고르는데, 모양과 값이 같다.
...
그달의 출근·지각·결근·출근 인정 넷을 한 줄에 센다. [domain/attendance.md](../../domain/attendance.md#세는-것)가 집계에 든다고 정한 넷이다.
...
| 세그먼트 | [payroll.md](payroll.md#기간-세그먼트)의 세그먼트 |
| 합계 | `fg.neutral` |
...
**세그먼트가 `components.md`에 없다.** [payroll.md](payroll.md#규칙과-부딪힌-자리)가 먼저 만난 자리고 이 화면이 두 번째 사용자다. 두 번 나왔으니 공용으로 올릴 자리인데, 두 화면의 시안을 그리고 값을 맞춰 본 뒤에 올린다.
