...
**hue를 40~80에서 멀리 뒀다.** brand가 57이라 그 언저리를 쓰면 차트 면이 브랜드처럼 읽힌다. 차트 계열이 서는 화면은 브랜드 버튼이 같이 서는 화면이라([foundation/color.md](foundation/color.md#차트가-색을-쓰는-법)) 두 색이 한 화면에서 갈려야 한다. chroma도 brand의 최대(0.058)와 비슷한 선에 묶었다 — 차트는 면이 크고 줄마다 반복되는데 브랜드보다 진하면 화면에서 제일 센 것이 그림이 된다.
