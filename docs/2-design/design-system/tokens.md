...
옅은 면 위의 조합 일곱은 [pages/login.md](../modules/account/screens/login.md#색-1)의 알림 영역에서 나왔다. 옅은 면에 글자를 올리는 자리는 알림 블록마다 되풀이되니 다른 화면에도 같은 값이 걸린다.
