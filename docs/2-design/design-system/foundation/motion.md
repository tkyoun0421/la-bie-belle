...
easing은 토큰으로 정하지 않았다. Tailwind 기본 `ease-out`을 쓴다. 들어올 때 빠르게 시작해 부드럽게 멈추는 곡선이고, 등장 애니메이션 대부분이 여기 맞는다. 끝없이 도는 것 둘 — 스켈레톤의 shimmer와 버튼 안 스피너 — 만 `linear`다. 시작과 끝이 없는 움직임에 가속이 붙으면 멈췄다 가는 것으로 보인다. 주기는 [tokens.md](../tokens.md#되풀이-주기와-계단)의 `--interval-shimmer`와 `--interval-spin`이다.
...
**스켈레톤의 shimmer도 그 예외다.** 화면이 데이터를 기다리는 동안 카드 모양 그대로의 회색 덩이 위로 빛이 은은히 흐른다. 탭을 옮길 때마다 보이는 자리라 위 목록에 걸리는데, 이것은 손짓에 따라붙는 장식이 아니라 기다리는 시간이 흐르고 있음을 말하는 움직임이다 — 멈춘 회색 덩이는 화면이 죽은 것과 갈리지 않는다. 내용이 오면 멈추고, 캐시가 있어 기다림이 없는 화면에서는 아예 안 뜬다. 주기는 `--interval-shimmer`고 조각은 [components.md](../components.md#skeleton)에 있다. 스피너는 버튼 안에서만 돈다 — 화면 가운데에 스피너를 세우지 않는다.
...
**움직이는 조각은 셋이고 예외가 하나다.** 시트와 팝오버와 토스트가 뜨고 지는 것, 버튼이 눌리는 것, 값이 바뀌며 채움이 움직이는 것이다. 예외는 기다림을 말하는 것들이다 — 스켈레톤의 shimmer, 버튼 안 스피너, 답을 기다리는 점선과 뛰는 점. 셋과 달리 상태가 바뀌는 순간이 아니라 바뀌기를 기다리는 동안 움직이고, 내용이 오면 멈춘다. 나머지는 안 움직인다 — [자주 일어나는 것은 움직이지 않는다](#자주-일어나는-것은-움직이지-않는다)가 그 목록이다.
