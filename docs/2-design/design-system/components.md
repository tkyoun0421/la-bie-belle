...
**목록을 접는 「더 보기」는 ghost다.** 목록 폭을 다 쓰고 `text-sm`이며, 눌리면 접힌 줄이 그 자리에 펼쳐지고 버튼은 사라진다. 시급 이력이 첫 자리다([pages/wages.md](pages/wages.md)) — 몇 줄부터 접을지는 화면마다 정한다.

## BottomCTA

화면 하단에 고정되는 주요 액션 버튼이다. 관리자 근무표의 「확정하기」와 근무자 근무 신청의 「보내기」가 여기다 — 화면을 다 채운 뒤 마지막에 한 번 누르는 것들이다.

**화면의 끝이 아닌 버튼은 여기 오지 않는다.** 그 버튼이 답하는 것이 화면 어딘가에 그려져 있으면 둘을 붙여 둔다. 대시보드의 출근 인증이 그렇다 — 하루 띠가 오늘 근무를 그리고 버튼은 그 근무에 대한 답이라, 화면 아래로 내리면 사이에 이번 주 근무와 급여가 끼어 왜 지금 눌리는지가 안 이어진다([pages/dashboard.md](pages/dashboard.md#규칙과-부딪힌-자리)).
...
**탭 바를 쓰는 허브 화면에서는 제목이 작다.** `text-sm`에 `fg.neutral-subtle`이고, 오른쪽에 브랜드 마크가 선다. 대시보드와 관리자 홈이 그렇다 — 앱바가 「여기가 어디인지」를 말할 필요가 적고, 그 아래 내용이 화면의 주인공이다. 관리자 홈은 탭 바가 없어 왼쪽에 뒤로가 서지만 제목은 그대로 작다([pages/schedule-admin.md](pages/schedule-admin.md#관리자-홈)). 그 밖에 뒤로 가기가 있는 화면은 위 표의 제목형을 쓴다.

제목 자리에 다른 것이 서는 화면이 둘 있다. 근무표는 달 이동 화살표와 누르면 달 고르기가 열리는 제목이고([pages/schedule-worker.md](pages/schedule-worker.md)), 대시보드는 날짜와 브랜드 마크다([pages/dashboard.md](pages/dashboard.md)). 둘 다 같은 높이와 여백을 쓴다.
...
**관리자 화면에는 탭 바가 없다.** 관리자 홈이 허브라서 거기서 갈라지고 앱바의 뒤로로 돌아온다([pages/schedule-admin.md](pages/schedule-admin.md#관리자-홈)). 관리자도 자기 근무가 있어 근무자 화면을 쓰는데, 그때는 탭 바가 있는 근무자 쪽에 서 있는 것이다.
...
같은 자료를 다른 단위로 보는 스위치다. 급여 조회의 달·주와 통계의 기간이 여기다([pages/payroll.md](pages/payroll.md), [pages/stats.md](pages/stats.md)). [Tabs](#tabs)와 다른 점은 화면이 안 바뀐다는 것이다 — 아래 내용이 다른 눈금으로 다시 서는 것뿐이다.
...
**가운데 Dialog**는 시트를 쌓지 않고 따로 뜨는 확인이다. 근무 취소 승인([pages/approvals.md](pages/approvals.md))과 시급 되돌리기([pages/wages.md](pages/wages.md))가 여기다. 시트 위에서 한 번 더 묻는 자리라 시트와 다른 모양이어야 한다.
...
그날의 근무를 가로축 하나로 눕히고 지금까지 얼마나 지났는지를 채운다. 근무표 날짜 칸처럼 색 규칙이 따로 붙는 조각이라 [foundation/color.md](foundation/color.md#근무표에서-색을-쓰는-법)와 양쪽에 걸친다. 대시보드가 첫 사용자고 상세는 [pages/dashboard.md](pages/dashboard.md#하루-띠)에 있다.
...
**인증 눈금은 찍힌 자리를 가리킨다.** 늦게 찍으면 그만큼 오른쪽에 서고, 정시거나 일찍 찍었으면 왼쪽 끝에 물린다. 지각이어도 색을 안 바꾼다 — 지각해도 급여는 그대로라([domain/attendance.md](../domain/attendance.md#용어)) 화면이 경고로 부를 일이 아니다. 안 찍었으면 눈금이 없다.
