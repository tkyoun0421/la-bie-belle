...
**모양은 알약이다.** `rounded-full`에 내용 폭이고 `shadow-card`로 떠 있다. 좌우로 늘리지 않는다 — 전폭 바에 짧은 문장이 들어가면 Button과 구분이 안 된다. 폭이 화면에 닿을 만큼 긴 문장은 좌우 24px 여백을 남기는 선에서 멈추고 줄을 바꾼다. 화면 아래에서 56px에 뜬다.
