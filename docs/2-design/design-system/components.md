...
**인증 눈금은 찍힌 자리를 가리킨다.** 늦게 찍으면 그만큼 오른쪽에 서고, 정시거나 일찍 찍었으면 왼쪽 끝에 물린다. 지각이어도 색을 안 바꾼다 — 지각해도 급여는 그대로라([domain/attendance.md](../../domain/attendance.md#지각)) 화면이 경고로 부를 일이 아니다. 안 찍었으면 눈금이 없다.

채움은 꽉 찬 브랜드 면이지만 누르는 것이 아니다. 브랜드가 한 화면에 여럿이어도 **누를 수 있는 브랜드는 하나**라는 [foundation/color.md](foundation/color.md#브랜드-색을-아끼는-법)의 결이고, 이 띠는 그중 안 눌리는 표식 쪽이다.
