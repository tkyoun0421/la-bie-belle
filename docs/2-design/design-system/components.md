...
**탭 바를 쓰는 허브 화면에서는 제목이 작다.** `text-sm`에 `fg.neutral-subtle`이고, 오른쪽에 브랜드 마크가 선다. 대시보드와 관리자 홈이 그렇다 — 뒤로 갈 곳이 없어 앱바가 「여기가 어디인지」를 말할 필요가 적고, 그 아래 내용이 화면의 주인공이다. 뒤로 가기가 있는 화면은 위 표의 제목형을 쓴다.
