...
관리자 편집 상태다. [pages/schedule-admin.md](pages/schedule-admin.md#달력-칸)가 더한 것을 여기로 합쳤다(2026-09-07). 그 문서의 「닫힌 날」은 이 표의 「안 연 날」과 같은 것이다.
...
브랜드 색으로 칸이 채워지는 것은 오늘 하루뿐이다. 예식이 있는 열린 날은 옅은 면으로 갈리고 브랜드 색은 점에만 쓴다(개정 2026-09-07 — 처음엔 `bg.neutral` + `shadow-card` + `stroke.surface` 카드였다. 시안 2차 검수가 볼록한 카드 대신 면 채움을 골랐다).

교육 배정은 근무 있음과 같은 칸이다(2026-09-07 — 처음엔 `fg.neutral-subtle` 점으로 갈랐는데 근무자 근무표 보기 검수가 구분을 걷었다). 교육인지는 칸이 아니라 명단과 목록의 글자가 말한다.
