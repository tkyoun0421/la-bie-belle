...
| --- | --- | --- | --- | --- | --- |
| `docs-sdlc-coherence` | SDLC를 유지하며 설계를 업무 영역으로 모은다 — [plan](3-build/plans/docs-sdlc-coherence.md). [제안](proposals/docs-sdlc-coherence.md) 채택, 근거는 [ADR-009](2-design/adr/ADR-009-design-modules-and-stage-links.md). 묶음 A~C가 끝났고 D~F는 아래 행이 잇는다 | active | — | [plan](3-build/plans/docs-sdlc-coherence.md) | — |
| `docs-authoring-playbook` | SDLC 문서의 작성법과 틀을 정본에 옮기고 활성 문서를 그 틀로 다시 쓴다 — [plan](3-build/plans/docs-authoring-playbook.md). [제안](proposals/docs-authoring-playbook.md) 채택, 근거는 [ADR-010](2-design/adr/ADR-010-authoring-guides-in-stage-readmes.md). 묶음 A~H를 PR 하나씩 나른다 | active | — | [plan](3-build/plans/docs-authoring-playbook.md) | — |
...
| `account-data-plan` | 계정 데이터 구조 전환의 plan을 쓴다 — `3-build/plans/account-data.md`. 정본은 [account/design.md](2-design/modules/account/design.md)·[account/design.md](2-design/modules/account/design.md). 완료 조건은 plan이 서는 것: `profiles.id` 분리와 `user_id → auth.users`, `profile_private`, 로그인 트리거 대신 `ensure_profile()`, 관리자 승인 함수, `update_my_photo()`까지 바꿀 마이그레이션·함수·integration 테스트 목록과 순서. 사진은 본인 변경 성공과 남의 변경 거부를 integration이 본다. plan이 서면 전환 task가 `blocked`에서 `ready`로 올라온다 | ready | — | 미작성 — 행이 완료 조건이다 | — |
...
| `types-generation` | 타입 생성 절차를 세운다 — `pnpm types`가 `supabase gen types`를 감싸고 CI가 마이그레이션 뒤 diff 0을 본다. 정본은 [system/data-access.md](2-design/system/data-access.md#읽기). 계정 데이터 구조 전환 뒤 | blocked | `account-data` | 미작성 — 선행 뒤 plan | — |
