...
| `members` | 직원 관리 화면을 만든다 — `/admin/members`. 검색, 퇴사 구획, 이름 고치기, 관리자 올리기·내리기, 퇴사 처리와 되돌리기. 정본은 [members.md](2-design/modules/account/screens/members.md). 가입 대기 뒤 — 공용 조각이 겹친다 | ready | `members-pending` | [plan](3-build/plans/members.md) | — |
...
| `schedule` | 근무표 — 달 만들기·날 열기·신청·확정. [schedule/design.md](2-design/modules/schedule/design.md). 남은 미정 둘(「배웠다」의 기준, `emit_reminders`의 주말 묶기)이 [schedule/design.md](2-design/modules/schedule/design.md#아직-안-정한-것)에 열려 있어 먼저 닫는다 | blocked | [schedule/design.md](2-design/modules/schedule/design.md#아직-안-정한-것) | 미작성 — 선행 뒤 plan | — |
...
| `sian-sync` | 재편 전부터 있던 문서↔시안 어긋남을 시안 쪽에서 맞춘다 — 값은 문서와 [tokens.md](2-design/design-system/tokens.md)가 정본이라 시안만 고친다. 자리는 재편 PR의 `sian-auditor` 결과가 든다: wages(#350 — 「시급」 라벨·제목 크기·`tabular-nums`·시트 패딩), approvals(#351 — 앱바 글자·거절 시트 여백 다섯·머리말 주석·「보내기 실패」 상태), qr·stats(#351 — 주석 속 옛 절 이름), schedule-admin(#352 — 없는 경로 `domain/schedule.md` 캡션), schedule-worker(#352 — 후보 없음 버튼 문구·「취소 요청 중」과 「보내기 실패」와 「배정이 사라졌을 때」 목업), excuse(#353 — 없는 경로 `domain/attendance.md`), dashboard(#353 — 「수락 취소」 CTA), check-in(#353 — 토큰 여섯 자리). 화면을 구현하는 task가 먼저 닿으면 거기서 같이 한다 | candidate | — | 미작성 — 행이 완료 조건이다 | — |
