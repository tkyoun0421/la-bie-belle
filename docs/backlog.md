...
| `profile-form` | 프로필 작성 폼을 만든다 — `/pending`에서 이름·연락처·생년월일·성별을 받아 `submit_profile`을 부르고, 「제출 안 함」·「승인 대기」·「거절된 뒤」를 가른다. `/left`의 짜임도 같이 그린다. 정본은 [login.md](2-design/modules/account/screens/login.md#프로필-작성)와 [login.md](2-design/modules/account/screens/login.md#퇴사한-뒤). `/blocked`의 짜임은 [아직 안 정한 것](2-design/modules/account/screens/login.md#아직-안-정한-것)이라 뺀다. 인증 진입 전환 뒤 | ready | `auth-entry` | 미작성 — plan | — |
