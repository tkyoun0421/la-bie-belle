...
| `auth-entry` | 인증 진입을 전환한다 — `middleware.ts`를 `proxy.ts`로, 서버 `readAuthGate`의 승인 판정을 클라이언트 `['profile']`로. 정본은 [account/design.md](2-design/modules/account/design.md). `matcher`를 이때 같이 본다. 계정 데이터 구조 전환 뒤 | ready | `account-data` | [plan](3-build/plans/auth-entry.md) | — |
