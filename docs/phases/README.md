...
P5는 P4 이후 시작합니다. P6는 P3 이후 시작할 수 있지만 공통 앱 셸과 알림 기반을 재사용하려면 P4 이후 P5와 병렬 진행하는 편이 효율적입니다. 한 에이전트는 한 번에 task 하나만 수행합니다.

## 상태 정의

- `planned`: 시작 가능 여부를 의존성으로 판단하는 대기 작업.
- `in_progress`: 현재 구현 중. 저장소 전체에서 하나만 권장.
...
2. `index.jsonl`의 task를 `in_progress`로 바꾼다.
3. Phase 문서의 범위 안에서만 구현한다.
4. task에 적힌 verification과 관련 회귀 테스트를 실행한다.
5. 인수 조건을 충족한 증거를 PR 또는 커밋 설명에 남긴다.
6. `index.jsonl` 상태와 `updated_at`을 갱신한다.
...
JSONL은 AI가 전체 문서를 읽기 전에 현재 작업과 관련 파일을 빠르게 찾는 용도입니다. 상세 요구사항은 반드시 연결된 Phase 문서와 PRD에서 확인합니다.
...
- 시각 디자인 변경: 향후 `DESIGN.md` 수령 후 별도 task로 추가.
