...
라비에벨 웨딩홀 근무자를 위한 웹 PWA다. 관리자가 월 근무표를 짜서 확정하면 근무자 폰에 뜨고, 근무자는 현장에서 출근을 찍고, 그 기록에서 앱이 급여를 계산한다. 네이티브로 안 가는 이유와 그 대가는 [ADR-007](../2-design/adr/ADR-007-web-pwa-over-native.md)에 있다.
