...
- `DEV-CODE-09` `MUST`: `ui` 세그먼트 파일은 표현(마크업·스타일·조립)과 이벤트 배선만 소유한다. 계산·변환·분기 판정·데이터 가공은 해당 slice의 `model`(화면 로직)·`lib`(일반 유틸)·`hooks`(React 상태·효과)·`entities`(도메인 규칙)에 두고 ui는 호출만 한다. 예외는 **className 조합과 열거값 분기** 둘뿐이다. `project/no-logic-in-ui`가 기계 강제한다.
