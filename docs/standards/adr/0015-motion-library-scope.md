...
번들이 제약이다. P0-T43이 정적 청크 gzip 합계 상한을 380KB로 세웠고 2026-08-09 기준선은 367KB였다. 여유가 13KB뿐이라 도입 여부보다 **어느 진입점까지 여느냐**가 실제 결정이 되었다.
...
P0-T43이 세운 380KB는 의존성이 0인 상태의 방어선이었다. `motion` 도입 후 예상치는 약 395KB이며 상한을 400KB로 조정한다. 여유 5KB는 의도한 압박이다 — 다음 task가 새 의존성을 들이려면 이 결정을 다시 열어야 한다.
...
- 번들 여유가 5KB로 좁아진다. 새 UI 의존성은 사실상 이 ADR을 다시 여는 일이 된다.
...
| 날짜 | 변경 | 승인 |
| --- | --- | --- |
| 2026-08-10 | 최초 채택. `motion` 13.0.0을 `LazyMotion` 범위로 도입하고 번들 상한을 400KB로 올린다. | user, 2026-08-10 |
