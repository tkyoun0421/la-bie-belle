...
이 표는 `motion`만 격리해 esbuild로 잰 값이며, **Turbopack 프로덕션 빌드에서는 재현되지 않는다.** `motion/react`는 `export * from 'framer-motion'`으로 77줄짜리 재수출 barrel을 통째로 다시 내보내는데, esbuild는 그 barrel을 흔들어 내고 Turbopack은 흔들지 않는다. 좁은 진입점으로 우회할 수도 없다 — `LazyMotion`과 `domAnimation`은 그 index에만 있고 `motion/react-m`은 `m.*` 요소만 준다. P0-T44 개발 중 이 저장소에서 잰 값은 아래와 같다(정적 청크 gzip 합계, 2026-08-10).

| 배치 | 실측 | 도입 전 대비 |
| --- | ---: | ---: |
| 도입 전 기준선 | 406.5KB | — |
| `motion/mini` — `animate()` | 412.8KB | +6.3KB |
| `motion/react` — `LazyMotion` + `domAnimation` | 477.5KB | +71KB |

`next.config.ts`에 `experimental.optimizePackageImports: ["motion"]`을 얹어도 바이트 단위로 같은 값이 나왔다. 진입점별 상대 순서는 표와 같지만 절대값은 격리 측정을 믿을 수 없다는 것이 이 실측의 교훈이다. 앞으로 번들 여유를 계산할 때는 저장소 빌드 실측을 쓴다.
...
P0-T44가 그 자리를 채워 **상한을 500KB로 정했다**(RADIO revision 4, 2026-08-10 사용자 결정). 근거는 위 실측이다. 27KB를 전제한 450KB는 성립하지 않았고, 프로바이더만 배치한 실측이 477.5KB였다. 상한을 두 번째로 올리는 것을 받아들인 이유는 이 수치가 예측이 아니라 실측이고, `motion`이 인증 뒤 화면 청크에만 실리며, 되돌림이 프로바이더 제거 하나로 끝나기 때문이다. 여유는 약 22KB이며, 도입 후 합계가 490KB를 넘으면 상한을 다시 올리는 대신 `motion/mini` 전환을 설계로 반환한다.

구현을 마친 뒤의 실측은 490.9KB(502,710바이트, gzip 정적 청크 38개)였다. 490KB 정지선을 950바이트 넘었지만 500KB 상한 안에 있다(RADIO revision 6, 2026-08-10 사용자 결정). 정지선의 목적은 "세 번째 상한 인상이 필요한 상황을 조기에 잡는 것"이었는데 세 번째 인상이 필요하지 않아 목적은 지켜졌다고 판단했다. 상위 세 청크(71.6KB·63.7KB·41.5KB) 어디에도 `framer-motion` 문자열이 없고 최대 청크는 `react-dom`이라 `motion` 쪽에서 더 줄일 자리가 없었다. `motion/mini` 전환은 `AnimatedAmount`가 쓰는 `useMotionValue`·`useSpring`·`useTransform`이 mini에 없어 배제했다.

교차 검증(F-01·F-02·F-03·F-04·F-10) 수정 라운드를 반영한 최종 실측은 **491.0KB(502,829바이트, gzip 정적 청크 38개)**다 — 정지선 초과분이 950바이트에서 1,069바이트로 119바이트(0.02%) 늘었을 뿐 500KB 상한 대비 여유 약 9.0KB는 그대로 유지된다.

지금 실측은 508KB(519,825바이트)다. 위 최종 실측 491.0KB에서 약 17KB가 늘었고 그 원인은 아직 안 셌다. P0-T48이 `@tanstack/react-query`와 `zustand`를 더하면 그 위에 더 벌어진다. `pnpm verify`가 `gate:bundle` 통과를 요구하므로 이대로면 P0-T48을 못 닫는다. **상한을 600KB로 올린다(RADIO revision 5, 2026-08-18 사용자 결정).** 이 문단은 바로 위 문단이 세운 「490KB를 넘으면 상한을 올리는 대신 `motion/mini` 전환을 설계로 반환한다」 조항을 뒤집는다 — 조용히 지우지 않는다. `motion/mini` 전환은 이미 배제된 대안이다(`AnimatedAmount`가 쓰는 `useMotionValue`·`useSpring`·`useTransform`이 mini에 없다). 이번 인상은 감시선을 다시 올린 것이지 17KB 증가분의 원인을 밝히거나 번들을 줄인 것이 아니다. 원인 규명과 감량은 후속 task(P0-T56)로 뺀다.
...
| 2026-08-10 | 결정 3을 채운다. 상한을 500KB로 정한다. Context의 esbuild 격리 측정이 Turbopack 빌드에서 재현되지 않아 저장소 실측 3종을 덧붙이고, 앞으로 여유 계산에 격리 측정을 쓰지 않는다고 못 박는다. 결정 1·2·4·5는 그대로다. | user, 2026-08-10 |
| 2026-08-10 | 결정 3에 구현 완료 실측을 기록한다. 최종 정적 청크 gzip 합계는 490.9KB(502,710바이트)로 490KB 정지선을 950바이트 넘었으나 500KB 상한 안이다. `motion/mini` 전환은 `AnimatedAmount`가 쓰는 훅 3종이 없어 배제했다. | user, 2026-08-10 |
| 2026-08-10 | 결정 3에 교차 검증 수정 라운드(F-01·F-02·F-03·F-04·F-10) 이후 실측을 갱신한다. 491.0KB(502,829바이트)로 이전 실측 대비 119바이트(0.02%) 늘었다. 500KB 상한 대비 여유는 그대로 약 9.0KB다. | user, 2026-08-10 |
| 2026-08-18 | 결정 3에 세 번째 인상을 기록한다. 상한을 600KB로 올린다. 490KB 정지선 조항을 뒤집으며, 그 조항이 지시한 motion/mini 대안은 이 ADR이 이미 배제했다. 원인 규명과 감량은 후속 task로 뺀다. | user, 2026-08-18 |
