...
export function recommendNextAction(progress: ProgressSummary): NextAction {
  ...
  if (running !== undefined) {
    return {
      ...
      taskId: running.id,
      rationale:
        "in_progress task는 저장소 전체에서 하나뿐이며, 끝내기 전에는 다음 task를 시작할 수 없다.",
  ...
  if (runnable !== undefined) {
    ...
    return {
      ...
      taskId: task.id,
      rationale:
        "두 승인과 실행 계약이 모두 있고 선행 task가 전부 done인 planned task 중 index 등록 순서로 가장 앞이다.",
      prerequisites: [
        `${task.id}의 RADIO 해시 확인(${task.radioRef ?? "radio_ref 없음"})`,
        task.dependsOn.length === 0
          ? "선행 task 없음"
          : `선행 task ${task.dependsOn.join(", ")} 완료 확인`,
      ],
      blockers,
  ...
  if (blocked !== undefined) {
    return {
      ...
      taskId: blocked.id,
      rationale:
        "실행 가능한 planned task가 없고 중단된 task가 있다. 결정 해소가 큐를 다시 여는 유일한 경로다.",
  ...
  if (designPending !== undefined) {
    return {
      ...
      rationale:
        "실행 큐가 비었고 기획 승인만 있는 task가 있다. RADIO 승인이 planned 진입 조건이다.",
      prerequisites: [
        "승인된 기획 범위 확인",
        "docs/execution/radio/<task-id>-radio.md 작성과 SHA-256 기록",
      ],
      blockers,
