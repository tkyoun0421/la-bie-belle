...
const { checkIndexStateRules } = await import(harnessLibUrl("index-gate.ts"));
...
try {
  ...
  if (watch) {
    const status = state.status === "waiting_rate_limit" || state.status === "armed" ? state.status : "running";
    state = { ...state, status, updated_at: new Date().toISOString() };
    mod.writeState(statePath, state);
    ...
    if (!mod.isSafeToInvokeClaude(violations)) {
      console.error(`repository reverification blocked the new session:\n${violations.map((v) => `- [${v.gate}] ${v.message}`).join("\n")}`);
      mod.writeState(statePath, { ...state, status: "needs_user", updated_at: new Date().toISOString() });
  ...
  while (true) {
    ...
    if (current.status === "waiting_rate_limit" && current.session_id && current.next_attempt_at && Date.parse(current.next_attempt_at) <= Date.now()) {
      try {
        ...
        writeFileSync(join(runtime, "unattended-resume.json"), `${JSON.stringify({ session_id: current.session_id, resumed_at: new Date().toISOString() })}\n`, { mode: 0o600 });
