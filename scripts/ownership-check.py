...
def prefix_role(mode, ref):
    ...
    role = BRANCH_PREFIX_ROLES.get(prefix)
    if role is None:
...
def worktree_role(mode, project_dir):
    ...
    return os.path.basename(os.path.realpath(project_dir))
...
def resolve_role(mode, project_dir):
    ref = os.environ.get("BRANCH_REF", "").strip()
    if ref:
        return prefix_role(mode, ref)
    ...
    declared = prefix_role(mode, branch)
    if declared != role:
        fail(
            mode,
...
def main():
    ...
    real_project = os.path.realpath(project_dir)
    role = resolve_role(mode, real_project)
    if mode == "hook":
