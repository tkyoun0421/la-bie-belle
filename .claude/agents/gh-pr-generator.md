...
원장을 쓴 작업이면 관련 Issue 절 아래에 `## Gates` 절을 만들고 **원장 전문을 코드 블록으로 붙여라**. `CHECK:`와 `EXPECT:`와 증거까지 그대로다. 요약으로 줄이지 마라 — 리뷰어가 `CHECK:`가 gate 제목이 말하는 것을 실제로 재는지 보는 자리다. 로컬 파일은 추적되지 않아 diff에 없다. 포기 줄이 있으면 함께 나른다.
