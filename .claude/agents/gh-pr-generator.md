...
3. 바뀐 모든 경로가 `config/ownership.json`에서 브랜치 접두의 역할에 속하는지 확인해라. 아니면 멈추고 보고해라 — 소유권 위반은 리뷰 실패이고, 여기서 잡는 게 싸다.
4. `orch/` 브랜치라면 registry가 못 잡는 검사를 하나 더 한다. `orchestrator` 키는 `["*"]`라 모든 것을 통과시킨다. 총괄은 pm·dev·ui 경로를 쓰는 대신 배정한다. 그 키들에 속한 경로가 바뀌었다면 3번을 통과했더라도 멈추고 보고해라.
5. `.env` 파일도 하드코딩된 자격 증명도 diff에 없는지 확인해라. 이 저장소는 공개다. 있으면 멈추고 보고해라. 커밋하지 마라.
6. 커밋하고 push한 다음 PR을 연다.
