...
5. `.gates/` 아래에 원장이 있으면 `python3 scripts/gate-check.py --reverify`를 돌려라. 미충족이 남으면 멈추고 보고해라 — 미충족인 채로 PR을 열지 않는다. 포기 줄은 미충족이 아니다.
...
원장을 쓴 작업이면 관련 Issue 절 아래에 `## Gates` 절을 만들고 **원장 전문을 코드 블록으로 붙여라**. `CHECK:`와 `EXPECT:`와 증거까지 그대로다. 요약으로 줄이지 마라 — 리뷰어가 `CHECK:`가 gate 제목이 말하는 것을 실제로 재는지 보는 자리이고, 로컬 파일은 추적되지 않아 diff에 없다. 포기 줄이 있으면 함께 나른다.
