...
1. **push**: pre-push 훅이 `pnpm build`를 실행하므로 환경 파일이 필요하다 — `cp .env.example .env && git push origin main; rm -f .env` 순서를 지키고 `.env`는 반드시 삭제한다(커밋 금지). `.env.example` 값은 공개 로컬 데모 값이다.
