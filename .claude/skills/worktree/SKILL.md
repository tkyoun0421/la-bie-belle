---
name: worktree
description: Orca가 관리하는 작업 트리를 열고 닫고 이름 바꾸고 목록을 본다. 트리를 열면 세션도 같이 띄운다. git worktree 명령은 안 쓴다. 트리거 — "작업트리 열어", "worktree 만들어줘", "트리 닫아", "트리 목록", "트리 이름 바꿔", "브랜치 따로 딸 트리", "세션 하나 더 띄워줘", "트리 정리해줘". 저장소 트리를 열고 닫고 이름 바꾸는 일은 여기가 맡고, 터미널 조작과 브라우저와 아티팩트 같은 나머지 Orca 조작은 orca-cli가 정본이다.
---
