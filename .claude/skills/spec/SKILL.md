...
1. `gh-issue-generator` — `[Epic]` Issue. `.github/ISSUE_TEMPLATE/epic.yml`이 묻는 칸을 그대로 채운다. 무엇을 만드나는 지금 어떻게 되어 있고 이 작업이 끝나면 무엇이 가능해지는지 한 문단, 무엇이 달라지나는 사용자가 겉으로 보는 변화를 항목으로, 이번에 하지 않는 것은 범위 밖으로 민 것이다. 상세 명세 칸에 스펙 파일 경로를 넣고, 인수 조건은 그 파일에 두고 복사하지 않는다. 라벨을 붙이고 카드를 보드에 올린다.
