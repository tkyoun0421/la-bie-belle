...
Epic 본문은 고치지 마라. 코멘트로 남긴다. 슬라이스가 전부 닫히면 **PM이** 최신 `origin/main`에서 이 gate를 돌리고 결과를 같은 Epic에 코멘트한 뒤 닫는다. 닫는 사람이 닫는 근거를 스스로 만드는 것이고, gate를 쓴 손(Dev)과 통과를 판정하는 손이 갈린다. 명령 실행은 쓰기가 아니라 소유 가드에 걸리지 않는다.
