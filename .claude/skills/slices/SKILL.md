...
이 대응은 쪼개는 자리에서 확인하고 Issue 본문에 옮겨 적지 않는다. 조건의 정본은 스펙 파일이고, `[Slice]` 폼에는 그걸 받는 칸이 없다 — "확인 방법" 칸은 직접 눌러 보는 순서를 받는 자리이지 조건을 복사하는 자리가 아니다. 슬라이스가 무엇을 맡았는지는 "이 슬라이스가 끝나면"이 사람 말로 말한다. Epic을 다 쪼갠 뒤 조건 목록을 한 번 훑어 빠진 것이 없는지 보고, 있으면 슬라이스를 더 잘라라.
