...
describe("requireAdmin", () => {
  it("admin 역할이 있으면 { ok: true, roles }를 반환한다", async () => {
    findOwnRoles.mockResolvedValue({ ok: true, data: ["worker", "admin"] });

    const { requireAdmin } = await import("@/entities/identity/api/require-admin");
    const result = await requireAdmin();

    expect(result).toEqual({ ok: true, roles: ["worker", "admin"] });
  });

  it("super_admin 역할도 admin으로 통과한다(계층 전개는 DB가 이미 admin을 포함한다)", async () => {
  ...
  it("admin 역할이 없으면 COMMON_FORBIDDEN으로 거부한다", async () => {
