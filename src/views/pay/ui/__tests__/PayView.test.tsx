...
describe("PayView", () => {
  it("이번 달 예상 합계와 예상 문구를 보여주고 급여·정산·지급 단어를 쓰지 않는다", () => {
    ...
    expect(screen.getByText("495,000원")).toBeInTheDocument();
    expect(
  ...
  it("일반 근무 예상액과 리허설 예상액을 나눠 보여준다", () => {
    ...
    expect(screen.getByText("리허설").closest("div")).toHaveTextContent("45,000원");
  });
  ...
  it("눈 아이콘을 누르면 금액을 숨긴다", async () => {
    ...
    expect(screen.queryByText("495,000원")).toBeNull();
  ...
  it("리허설 기록 추가를 누르면 입력 바텀시트가 열리고 추가하면 목록에 반영된다", async () => {
    const user = userEvent.setup();
    render(<PayView {...PAY_EMPTY_MONTH} />);
  ...
  it("기존 기록을 탭하면 수정·삭제 바텀시트가 열리고 삭제하면 목록에서 사라진다", async () => {
    const user = userEvent.setup();
    render(<PayView {...PAY_WITH_ITEMS} />);
