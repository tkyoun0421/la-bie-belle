...
export type RevokeAdminResult = { ok: true } | { ok: false; code: ErrorCode };

export async function revokeAdmin(targetProfileId: string): Promise<RevokeAdminResult> {
  ...
  if (error) {
    process.stderr.write(
      `${JSON.stringify({ event: "identity_revoke_admin_failed", code: error.code })}\n`,
    );
