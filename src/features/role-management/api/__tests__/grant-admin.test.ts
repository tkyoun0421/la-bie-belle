...
describe("grantAdmin", () => {
  it("super_admin이면 grant_admin_role RPC를 호출하고 목록 경로를 재검증한다", async () => {
    requireSuperAdmin.mockResolvedValue({ ok: true, roles: ["worker", "admin", "super_admin"] });
    rpc.mockResolvedValue({ data: null, error: null });

    const { grantAdmin } = await import("@/features/role-management/api/grant-admin");
    const result = await grantAdmin("target-1");

    expect(result).toEqual({ ok: true });
    expect(rpc).toHaveBeenCalledWith("grant_admin_role", { target_profile_id: "target-1" });
  ...
  it("RPC 오류가 있으면 COMMON_UNEXPECTED로 거부하고 재검증하지 않는다", async () => {
    requireSuperAdmin.mockResolvedValue({ ok: true, roles: ["worker", "admin", "super_admin"] });
    rpc.mockResolvedValue({ data: null, error: { code: "22023", message: "not active" } });
