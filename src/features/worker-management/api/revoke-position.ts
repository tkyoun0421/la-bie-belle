...
export type RevokePositionResult = { ok: true } | { ok: false; code: ErrorCode };

export async function revokePosition(
  targetProfileId: string,
  positionId: string,
): Promise<RevokePositionResult> {
  ...
  const { error } = await supabase.rpc("revoke_position_eligibility", {
  ...
  if (error) {
    process.stderr.write(
      `${JSON.stringify({ event: "identity_revoke_position_failed", code: error.code })}\n`,
    );
