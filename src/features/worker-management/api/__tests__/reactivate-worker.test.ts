...
describe("reactivateWorker", () => {
  it("admin이면 reactivate_worker RPC를 호출하고 상세를 재검증한다", async () => {
    requireAdmin.mockResolvedValue({ ok: true, roles: ["worker", "admin"] });
    rpc.mockResolvedValue({ data: null, error: null });

    const { reactivateWorker } = await import("@/features/worker-management/api/reactivate-worker");
    const result = await reactivateWorker(TARGET_ID);

    expect(result).toEqual({ ok: true });
    expect(rpc).toHaveBeenCalledWith("reactivate_worker", { target_profile_id: TARGET_ID });
  ...
  it("ID가 UUID가 아니면 RPC 없이 검증 실패를 반환한다", async () => {
    ...
    const { reactivateWorker } = await import("@/features/worker-management/api/reactivate-worker");
    const result = await reactivateWorker("not-a-uuid");
  ...
  it("상태 부적합 전이는 매핑된 코드로 거부하되 재검증은 수행한다", async () => {
    requireAdmin.mockResolvedValue({ ok: true, roles: ["worker", "admin"] });
    rpc.mockResolvedValue({ data: null, error: { code: "LB011", message: "invalid" } });
