...
export type DeactivateWorkerResult = { ok: true } | { ok: false; code: ErrorCode };

export async function deactivateWorker(targetProfileId: string): Promise<DeactivateWorkerResult> {
  ...
  const { error } = await supabase.rpc("deactivate_worker", {
  ...
  if (error) {
    process.stderr.write(
      `${JSON.stringify({ event: "identity_deactivate_worker_failed", code: error.code })}\n`,
    );
