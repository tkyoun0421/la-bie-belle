...
describe("OwnWageForm", () => {
  it("저장값이 있으면 그 값을 입력 필드에 채운다", () => {
    render(<OwnWageForm action={vi.fn()} initialAmount={13000} isDerived={false} />);

    expect(screen.getByLabelText("시급")).toHaveValue(13000);
  });

  it("파생 표기(isDerived)면 기본 시급 금액을 입력 필드에 그대로 보여준다(F-06)", () => {
    render(<OwnWageForm action={vi.fn()} initialAmount={12000} isDerived />);
  ...
  it("저장 버튼을 누르면 action이 호출된다", async () => {
    ...
    render(<OwnWageForm action={action} initialAmount={13000} isDerived={false} />);
