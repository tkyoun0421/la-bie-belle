...
const CONFETTI: {
  ...
  { top: "-6%", left: "42%", tone: "bg-bg-brand-solid", round: true },
  { top: "4%", left: "-4%", tone: "bg-bg-brand-muted", round: false },
  { top: "6%", left: "94%", tone: "bg-stroke-neutral-muted", round: true },
  { top: "44%", left: "-10%", tone: "bg-bg-brand-solid", round: false },
  { top: "46%", left: "102%", tone: "bg-bg-brand-muted", round: true },
  { top: "88%", left: "2%", tone: "bg-stroke-neutral-muted", round: false },
  { top: "96%", left: "50%", tone: "bg-bg-brand-solid", round: true },
  { top: "86%", left: "92%", tone: "bg-bg-brand-muted", round: false },
];
...
export default function Screen() {
  ...
  if (stage === "loading") {
    return <View className="flex-1 bg-bg-neutral" />;
  }
  ...
  if (stage === "celebrating") {
    return (
      <View className="flex-1 items-center justify-center bg-bg-neutral px-6">
        <View>
          <Avatar name={values.name} photoUrl={values.photoUrl} size={64} />
          {CONFETTI.map((piece) => (
            <View
              key={`${piece.top}-${piece.left}`}
              style={{ top: piece.top, left: piece.left }}
              className={`absolute size-1.5 ${piece.tone} ${
                piece.round ? "rounded-full" : "rounded-xs"
              }`}
            />
          ))}
        </View>
        <Text className="mt-6 font-semibold text-2xl text-fg-neutral">
          {values.name}님, 반가워요
        </Text>
      </View>
    );
    ...
    return (
      ...
      >
        <View className="flex-1 items-center justify-center">
          {rejected ? (
            <Badge variant="neutral" label="아직 연결 전" />
          ) : (
            <>
              <Illustration scene="waiting" />
              <View className="h-8 flex-row items-center gap-2 self-center rounded-sm bg-bg-brand-weak px-3">
                <View className="size-1.5 rounded-full bg-bg-brand-solid" />
                <Text className="font-medium text-xs text-fg-brand">
                  승인 기다리는 중
                </Text>
              </View>
            </>
          )}
          <Text className="mt-4 text-center font-bold text-xl text-fg-neutral">
  ...
  const showGuide = (step: Step): string | undefined =>
    touched.includes(step) ? errors[step as "birthDate" | "phone"] : undefined;
  ...
  return (
    ...
    >
      <AppBar
        ...
        right={
          <Pressable accessibilityRole="button" onPress={onSignOut}>
            <Text className="text-sm text-fg-neutral-subtle">로그아웃</Text>
          </Pressable>
        }
      />

      <View className="flex-1 px-6">
        <Text className="text-sm text-fg-neutral-subtle">
          {open === null
        ...
        <View className="mt-6">
          ...
          {isFrozen("name") ? (
            <Pressable
              ...
              className="mt-6"
            >
              <Text className="font-semibold text-xl text-fg-neutral">
                {values.name}님, 반가워요
              </Text>
            </Pressable>
          ) : null}
          ...
          {isFrozen("gender") && values.gender ? (
            <Pressable
              ...
              className="mt-3"
            >
              <Text className="font-medium text-lg text-fg-neutral">
          ...
          {isFrozen("birthDate") ? (
            <Pressable
              ...
              className="mt-3"
            >
              <Text className="font-medium text-lg text-fg-neutral tabular-nums">
        ...
        {open === "photo" ? (
          <View className="mb-4 items-center">
            ...
            {photoFailed ? (
              <Text className="mt-1.5 text-xs text-fg-critical">
        ...
        {open === "gender" ? (
          <View className="mb-4">
            <Text className="mb-1.5 text-xs text-fg-neutral-muted">성별</Text>
            <Segment
              options={[
        ...
        {open === null ? (
          <Text className="mb-4 text-xs text-fg-neutral-subtle">
      ...
      <BottomCTA
        note={
          submitFailed ? (
            <Text className="text-center text-xs text-fg-critical">
              보내지 못했어요. 다시 시도해주세요
            </Text>
          ) : open === null ? null : (
            <Text className="text-center text-xs text-fg-neutral-subtle">
              빈 칸을 다 채우면 보낼 수 있어요
            </Text>
          )
        }
