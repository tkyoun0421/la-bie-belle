...
const [SCHEDULE_ROSTER_BAND_A, SCHEDULE_ROSTER_BAND_B] = splitBand(
...
test.describe("확정 배정표", () => {
  test("AC8: 확정 상세는 포지션 그룹·내 배정·교육 구분을 보여주고, 그날 미배정 근무자는 내 배정 섹션이 없다", async ({
    ...
    const workDate = workDateInBand(SCHEDULE_ROSTER_BAND_A);
  ...
  test("AC10: OPEN 스케줄을 URL로 직접 열면 배정표 없이 모집 중 안내가 나온다", async ({
    ...
    const workDate = workDateInBand(SCHEDULE_ROSTER_BAND_B);
